...
test('BGM and battle sound assets are valid on-demand MP3 files outside startup precache', () => {
  ...
  const damage0Url = new URL('../../assets/audio/damage0.mp3', import.meta.url);
  const status03Url = new URL('../../assets/audio/status03.mp3', import.meta.url);
  const status04Url = new URL('../../assets/audio/status04.mp3', import.meta.url);
  ...
  assert.equal(isMp3(damage0Url), true);
  assert.equal(isMp3(status03Url), true);
  assert.equal(isMp3(status04Url), true);
  ...
  assert.ok(statSync(damage0Url).size < 100_000);
  assert.ok(statSync(status03Url).size < 100_000);
  assert.ok(statSync(status04Url).size < 100_000);
  ...
  assert.doesNotMatch(precache, /damage0\.mp3/);
  assert.doesNotMatch(precache, /status03\.mp3/);
  assert.doesNotMatch(precache, /status04\.mp3/);
