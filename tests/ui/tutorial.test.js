...
test('home lobby keeps the release version visible and separates its navigation labels', () => {
  ...
  assert.match(css, /\.home-app-version\.home-lobby-version\s*\{[^}]*position:absolute;[^}]*z-index:12;[^}]*background:rgba\(2,9,17,\.72\)/s);
  ...
  for (const label of ['サバイバル', 'アリーナ', 'ホーム', 'カード', 'ショップ']) assert.match(homeSource, new RegExp(`label: '${label}'`));
  assert.match(homeSource, /王座への挑戦者、求む/);
  ...
  assert.match(css, /\.home-artwork-fullscreen > img\s*\{[^}]*object-fit:cover;/s);
