...
特殊合体は `specialFusionId` で36セルの専用アトラスへ切り替わります。ブルードリルだけは `blue-drill-v2.jpg` を使用します。正方形セルは中央で縦横比を維持し、縦長カードの外周だけ同じセルのcover画像で補完するため、引き伸ばしと個体ずれを防ぎます。Training・修行・既存ブリーダー20枚は25セルの専用アトラスを使います。追加ブリーダー20枚は`assets/images/breeders/breeder-021.jpg`〜`040.jpg`の個別アートです。名称・コスト・詳細効果はすべて動的HTMLです。
