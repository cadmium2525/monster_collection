...
特殊合体は `specialFusionId` で36セルの専用WebPアトラスへ切り替わります。ブルードリルだけは `blue-drill-v2.webp` を使用します。カード全面のアトラス背景1枚だけを表示し、中央疑似画像は生成しません。Training・修行・既存ブリーダー20枚も25セルの専用WebPアトラスを単一レイヤーで使います。追加ブリーダー20枚は`assets/images/breeders/breeder-021.webp`〜`040.webp`の個別アートです。名称・コスト・詳細効果はすべて動的HTMLです。ゲーム内ランタイム画像はすべてWebPで、PWA導入互換性用の小容量PNGアイコンと編集元SVGだけを例外として維持します。
