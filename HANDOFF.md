...
バトル画面は左ステータス・中央テーブル・右ログの3列です。カードは全面イラストで、提供された透過WebPのLIFE=左上ハート、コスト=右上円、ATK=左下剣、DEF=右下盾を外周へ重ね、特性・効果は詳細画面へ集約しています。LIFE・ATK・DEFは同寸で、コストだけ一段小さい確定サイズです。アタッカー・バランス・タンクの役割アイコンはカード面へ表示せず、詳細画面で役割名を確認できます。手札や保存一覧のモンスター名はカード最上部に表示し、縮小される盤面カードでは名前帯を省略します。アクセシブル名、詳細画面、ログには名前を残しています。名前帯による頭部隠れを避けるため、背景の上へ比率を保った本体画像を頭部セーフエリア付きで重ねています。大会内成長は四隅の実数値へだけ反映し、「大会+10」等の補助バッジは表示しません。手札上部の枚数・操作案内・山札/墓地行は撤廃し、その高さをカード拡大へ使用しています。山札・墓地は左HUDで確認できます。相手カードもプレイヤーから読める正立表示です。行動一覧はなく、手札を一度タップしてから合法対象へスワイプします。盤上モンスターは詳細内の実戦技を選び、盤面上の対象をタップします。高さ430px以下ではカードを高さ基準で縮尺しつつ、HUD・ログ・操作文字は10px前後を維持します。Pointer Eventsは選択済みカードだけで開始し、バトル中のbody/appはviewportへ固定しています。タッチ端末ではhover/選択によるカード位置移動を発生させません。
...
特殊合体は `specialFusionId` で36セルの専用WebPアトラスへ切り替わります。ブルードリルだけは `blue-drill-v2.webp` を使用します。正方形セルは中央で縦横比を維持し、縦長カードの外周だけ同じセルのcover画像で補完するため、引き伸ばしと個体ずれを防ぎます。Training・修行・既存ブリーダー20枚は25セルの専用WebPアトラスを使います。追加ブリーダー20枚は`assets/images/breeders/breeder-021.webp`〜`040.webp`の個別アートです。名称・コスト・詳細効果はすべて動的HTMLです。ゲーム内ランタイム画像はすべてWebPで、PWA導入互換性用の小容量PNGアイコンと編集元SVGだけを例外として維持します。
