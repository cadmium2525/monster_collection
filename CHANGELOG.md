# Changelog

## 1.41.0 — 2026-09-13

- 効果音ラボの無料SEを9点追加。メニュー操作、召喚、合体・覚醒の結果出現、パック開封とレア公開、勝敗、ミッション・ギフト・ランク報酬受取に適用。台詞・合成音声は不使用。
- 既存の攻撃・ドロー・ステータス変化SEを維持し、追加音も共通のSE音量・非表示時停止・iOS安全制御に統合。操作連打の音を抑制し、追加音声はオンデマンド読込（合計約462KiB）。
- ゲーム内「遊び方」と音声ディレクトリのCREDITS.mdに出典・利用条件を記載。

## 1.40.4 — 2026-09-13

- iPhone/PWAでBGMが曲末に停止する場合に備え、全BGMへループ属性と曲終了時の先頭再生処理を追加。ホーム・大会前・バトルの各BGMが継続してループするようにした。

## 1.40.3 — 2026-09-12

- デイリーミッション不具合の原因特定に使用した「判定情報」表示と一時的な診断収集を撤去。端末に残った診断履歴も起動時に削除する。

## 1.40.2 — 2026-09-12

- Firestoreのeconomy保存を再帰的マージからフィールド単位の置換へ変更。日付更新や修復で除いた旧デイリー試合数・勝利数がクラウドに残り、再ログインで復活する原因を修正。
- ミッションschema v6で旧版の未受取デイリー試合数・勝利数を一度リセット。受取済み報酬と週次・月次進捗は維持。更新当日の未受取実績は再達成が必要になる場合がある。
- テスト用Firestoreの再帰的マージを実際の仕様に合わせ、修復後の再読込・日付更新・当日の正当な勝利維持を検証。

## 1.40.1 — 2026-09-12

- ミッション画面に「判定情報」と報告用コピーを追加。当日の達成判定値、保存前後・クラウド同期・保留操作の再実行を直近80件まで端末内に記録し、再ログイン後の誤達成を調査できるようにした。報酬判定自体は変更せず、アカウント名・ID・メール・認証情報は記録しない。

## 1.40.0 — 2026-09-10

- アリーナの対戦相手選択へ「自分で操作／オートバトル」を追加。オート時は自分と相手のデッキを同じランクのAIが操作し、オート向けのデッキ構築を新しい選択肢として遊べるようにした。
- オート戦は双方の代表モンスターを左右へ大きく表示し、プレイヤーLIFEとゲージの増減を短い観戦演出として再現する。演出スキップと中断保存にも対応。
- オート戦も通常アリーナと同じBattleEngine、レート、報酬、ミッション処理を使用し、結果を二重計上しない。
- オート戦のマリガンと全合法手を小さな決定的トランスクリプトとして保存。結果画面の「試合内容を見る」から、通常のバトル画面と演出で試合をリプレイできる。

## 1.39.4 — 2026-09-10

- アリーナ等からホームへ戻った際、iPhone/PWAでホームBGMが再開しない問題を修正。WebKitのユーザー操作制約に合わせて再生開始順を見直し、モード内の読込中は現在の曲を維持する。
- アリーナ／トーナメント／サバイバルの試合前・結果画面とバトルを除き、ミッション、マイページ、ショップ、カード管理など全ユーティリティ画面でホームBGMを継続する。
- 機鋼の蓄圧装甲をDEF+10・3ターンとし、複数被弾で合計10まで貯められるよう変更。装甲解放を+15%・蓄圧5以上でTP軽減へ調整し、0ダメージでは装甲を消費しない。
- 魔族の血脈点火をLIFE2消費・+25%へ、血債回収をTP1・40%吸収・連携時行動権+1へ調整。AIは自傷分を回収できる攻撃とTPが揃う場合だけ、同じ攻撃役へ連続使用する。
- 上位CPUデッキは特殊合体後の分類を決めるメイン側がデッキ分類と一致するレシピを優先し、余剰素材から削ることで機鋼・魔族の主分類密度と分類専用カードの有効対象を維持する。
- オート戦監査の焦点型デッキをモンスター16・ブリーダー4・育成20枚へ是正。未使用seed 224戦を記録し、共通Legend環境51.0%まで改善したものの導入基準未達のため、アリーナのオート戦は引き続き保留する。

## 1.39.3 — 2026-09-09

- Gold／Legend／Champion AIへ、エース特殊合体の素材保護と同一ターンのTP予約を追加。最後の素材を召喚・通常合体・覚醒・手札コストへ流用せず、完成可能なら主役召喚から融合強化指示・特殊合体までを優先する。
- 分類コンボは準備札・回収札・攻撃に必要なTP、行動権、対象状態が揃った時だけ開始。融合強化指示は特殊合体前、素材探索は不足しているエース素材を優先し、防御起点コンボは実際に条件を満たせる公開盤面でのみ使用する。
- 同数の特殊合体候補は性能指数を加味してエースを選ぶよう改善。魔族は点火・血債回収・攻撃を同じ個体で行い、自傷分を回収できない小ダメージではコンボを消費しない。
- 固定seedのアリーナ・オート戦監査へ、エース完成数、コンボ準備／回収数、変換率の計測を追加。採用版336戦で成熟デッキの共通環境差は+2.1ptに留まったため、アリーナ・オート戦はHOLD継続とした。

## 1.39.2 — 2026-09-09

- 通信失敗で保留された前日の試合結果が、登録済みアカウントの次回起動時に当日のデイリー「1試合」「1勝」へ加算される問題を修正。進行操作へ発生日を保存し、既存の保留操作も保存時刻から正しい日付を復元する。
- サバイバルの挑戦デッキ選択で、代表カードのLIFE／TP／ATK／DEFアイコンを非表示に変更。
- サバイバルの任意ラン終了を廃止。試合中はトーナメント／アリーナ／サバイバル共通の「中断してホーム」で盤面を保存し、各モードから続きへ戻れるよう変更。

## 1.39.1 — 2026-09-08

- SEを音源別の小規模な再利用プールへ変更し、長時間の連戦で音声要素が増え続けてSEが再生されなくなる問題を防止。
- ターン演出SEだけを通常SE比130%へ調整。ステータス上昇／下降SEはカード吸収の着弾時、その他は数値アニメーション開始前に鳴るよう前倒し。
- お知らせへサバイバルモード開幕案内を追加し、ギフトボックスへ期限なし・一度限りの1,500ダイヤ記念ギフトを追加。

## 1.39.0 — 2026-09-08

- 第三の常設対戦コンテンツ「サバイバル」を追加。トーナメントやアリーナで作った保存40枚を固定し、Silver、Gold、Legendへ段階上昇するCPUと連戦できるようにした。
- Training・修行の能力成長、習得技／実戦技、残りプレイヤーLIFEをラン内で持ち越し、5勝ごとにLIFEを全回復。第3戦以降のLegend CPUも連勝数に応じた仮想大会育成を無上限で受け、長期ランの難易度が頭打ちにならないよう設計。
- 敗北または任意終了時に連勝数別のダイヤ／パック券を冪等付与し、自己ベスト更新の5勝刻みボーナス、総ラン数／総勝利数、オンライン最高連勝ランキングを追加。
- トーナメント・アリーナ・サバイバルのチェックポイントをモード別に分離。1つを中断したまま他モードを遊べ、各コンテンツへ個別に復帰できるよう変更。
- ホーム下部のトーナメント枠をサバイバルへ変更し、トーナメントは現チャンピオン情報下の「王座への挑戦者、求む」から開始／再開する導線に整理。
- サバイバル用Firestore Rules／複合index、固定seedで対戦結果まで一致する検証CLI、報酬・LIFE回復・独立中断・横向き一画面表示のテストを追加。

## 1.38.4 — 2026-09-08

- クラウド書き込み失敗時も端末側のデッキ・所持データ・プロフィールを正本として保持し、保留操作を端末へ保存して次回接続時に冪等再送するよう変更。古いクラウド値で新しい端末データが巻き戻る問題を防止。
- Firebase初期化・読込・再開データ更新へ共通タイムアウトとローカル復帰を適用し、通信停止時にローディング画面から進まなくなる経路を解消。
- 試合中の端末チェックポイントは毎行動維持しつつ、連続するクラウド保存は最新状態へ集約。Firestore保存直後の再読込と描画・AI評価時の不要な全ログ複製も削減。
- PWA初回キャッシュからブリーダー詳細絵・特殊合体詳細絵・1024pxマスクアイコンを外し、先読みを約18.3MB／247件から約5.5MB／136件へ削減。同一リリースのJSもクエリ違いで二重取得しないよう修正。
- ミッション・アリーナ画面の高さへ同期済み表示領域を使用し、844×390相当の横向き画面で余計な縦スクロールや下端のはみ出しが起きないよう調整。

## 1.38.3 — 2026-09-08

- 提供音源 `damage0.mp3`、`status03.mp3`、`status04.mp3` を追加。攻撃技が0ダメージだった場合は専用音を衝突フレームで再生し、非攻撃技では鳴らさないよう判定。
- LIFE・ATK・DEF・TPの変動アニメーションへ上昇／低下SEを同期。同じ効果で複数値が動く場合は1音へ集約し、上昇と低下が混在するときは上昇音を優先。
- 攻撃ダメージによるLIFE低下と通常のTP消費はステータス低下音から除外し、攻撃音との二重再生や毎行動の不要な低下音を防止。
- 寄生根はプレイヤー視点で判定し、プレイヤー側プラントの吸収は上昇音、CPU側プラントの吸収は低下音を再生。

## 1.38.2 — 2026-09-08

- 提供音源 `hit.mp3`、`turn.mp3`、`card_se.mp3` を共通SEチャンネルへ追加。ターン交代演出、初期手札・引き直し・通常／効果ドロー、素材探索の獲得時に演出と同期して再生。
- 攻撃SEは実行前の複製バトルで結果を検算し、モンスターまたはプレイヤーのLIFEへ1以上の実ダメージが入る攻撃だけ、衝突フレームで再生。0ダメージ攻撃は無音のまま維持。
- ショップ、パック開封、保存デッキ、デッキ編集、未所属カード、カード図鑑ではホームBGMを停止・再開せず、そのまま同じ再生位置で継続するよう変更。
- 追加SEは既存のSE音量0〜100と共通マスター出力50%を適用し、PWAの初回事前キャッシュには含めず必要時に読み込む構成を維持。

## 1.38.1 — 2026-09-08

- 提供音源 `arena.mp3` を、アリーナのデッキ／対戦相手選択画面とトーナメントの大会選択／大会表で流れる試合前BGMとして追加。
- 新しい試合前BGMは共通マスター出力50%の対象とし、画面上のBGM設定0〜100をその範囲内で適用。試合開始時は既存のバトルBGMへ切り替える。
- BGMファイルをPWAの初回事前キャッシュから除外し、必要な画面で初めて読み込むことで、約5.2MBの追加音源が起動時間を延ばさないよう調整。

## 1.38.0 — 2026-09-08

- 保存デッキ編集を、使用中カード4列・常時表示のカード効果・入替候補4列の3ペイン構成へ刷新。取得可能な全9技を初期技／攻撃修行／防御修行に分け、効果を読みながら交換元と候補を確定できるよう変更。
- 同一カードは通常絵・Foil・特別絵を混同せず外観単位で束ね、枚数表示で40枚を把握しやすく改善。使用中カードと入替候補へ独立した絞り込み・ソートを追加。
- デッキ編集内の四隅ステータス表示を省き、カード名帯の下からイラストを描画して、名前ラベルが顔や上部イラストへ重なる問題を修正。
- 登録済みアカウントで旧v3の日次データに混入した累積試合数・勝利数が、ログイン報酬受取後の再起動で復活する問題をv4移行で修復。受取済み報酬は維持し、未実施の当日試合・勝利だけを未達へ戻す。

## 1.37.3 — 2026-09-07

- iPhone PWAを最初から横向きで起動した際、補正後のホームが古い固定描画領域で切られ、下部メニューが画面外へずれる問題を修正。
- ホーム、背景、下部メニュー、バージョン表記を同一の同期済み画面高へ所属させ、画面下端まで一体で描画するよう変更。
- 登録済みアカウントのミッション受取時、クラウドトランザクションに残った古い日次カウンターが復活する問題を修正。受取直前に表示されていた同一期間の進捗を固定し、受取状態と報酬だけを更新。
- 日付をまたいで遅延したクラウド更新も、端末の現在日ではなく操作に記録された日本日付で日次・週次・月次ミッションを処理するよう修正。

## 1.37.2 — 2026-09-06

- iPhoneのホーム画面追加版を最初から横向きで起動した際、WebKitが一時的に短い表示領域を返してホーム下部が押し上がる問題を修正。
- iOS standalone・横向きに限って端末寸法も参照し、起動直後から実画面高をCSSへ同期。描画直後の安定化処理と回転・復帰時の再同期も追加。
- 通常ブラウザ、PC版PWA、縦向き警告画面には端末寸法補正を適用せず、既存レイアウトを維持。

## 1.37.1 — 2026-09-06

- 上位AIが危険な召喚候補を評価する際、公開盤面だけから相手の攻撃技・対象選択・TP・複数行動を組み合わせた最善攻撃列を先読みするよう変更。
- 丈夫な既存モンスターがいる状況で、倒しやすい追加モンスターを召喚して超過ダメージや敗北を招く行動を回避。盤面が空で召喚が唯一の防御になる場合は従来どおり召喚可能。
- 致死超過、明確な非致死超過、唯一の壁が必要な状況をGOLD・LEGEND・Champion AIでそれぞれ自動検証。

## 1.37.0 — 2026-09-06

- 分類間に共通で掛かっていた有利・不利の攻撃補正（実装値1.25倍）を撤廃。分類はデッキのコンボ方針を示す役割とし、個別相性はカード本文に書かれた効果だけに限定。
- Bランク相当以上のAIが、公式デッキ・プレイヤーゴースト・レジェンドアーカイブを区別せず、実際の40枚から主分類・バランス型・特殊合体ルート・コンボ部品・エース素材を解析するよう変更。
- AIが完成可能な特殊合体を優先し、エース素材の無駄遣いを避け、準備カードから回収カードへ繋げる分類別の行動計画を追加。D/C帯は従来の軽量な判断を維持。
- 幻霊「残像追撃」をTP1へ変更。魔族「血脈点火」はLIFE3支払い・ダメージ+20%・常時TP-1へ、「血債回収」は30%吸収（最大12）・自傷連携時+25%へ調整。
- 分類別CPUデッキを総当たりさせ、勝率・先後差・特殊合体・カード使用数を再現可能に集計する `npm run sim:factions` を追加。

## 1.36.0 — 2026-09-06

- 6分類それぞれに、準備と回収が連動する新規コンボ用ブリーダーカードを2枚ずつ、合計12枚追加。
- 魔族「血脈点火」はLIFE5支払い後の現在値が最大LIFEの50%以下なら強化される仕様とし、すでに50%以下の場合も条件を満たすよう明確化。
- 蓄圧・調律・残像・血債・狩場・残滓などの特殊状態を、モンスターカード右側の固定レールへ最大3件と超過件数で表示。
- GOLD・LEGENDの分類別CPUデッキへ対応するコンボ2枚をセット採用しつつ、従来の分類専用カードも候補として維持。
- 新規12枚の通常イラストと図鑑用軽量サムネイルを追加し、一覧表示ではフル画像を一括読込しない構成を維持。

## 1.35.14 — 2026-09-06

- 登録済みアカウントでクラウドの最終ログイン日だけが先に保存され、当日のログインミッション進捗が欠けた場合に、起動時に進捗を自己修復するよう変更。
- 自己修復時はマンスリーのログイン日数を二重加算せず、ログイン報酬300ダイヤを通常どおり受け取れるよう修正。
- 不整合な受取操作IDが残っている場合でも、未受取の達成済みミッションを受け取れるよう冪等処理を強化。

## 1.35.13 — 2026-09-06

- ミッション報酬の受取処理が進捗カウンターを一切変更できないよう、受取済み状態と報酬付与だけを独立して更新する方式へ変更。
- 旧版で保存された「未受取なのにデイリー試合・勝利が達成済み」の不整合を一度だけ補正し、受取済み報酬・週間・月間進捗は維持。
- 同じ受取操作IDの再実行を無効化し、端末保存とクラウド同期をまたぐ重複処理を防止。

## 1.35.12 — 2026-09-05

- 自分のターン開始時の通常ドローから、「CARD DRAW」「山札から○枚引きます」等の説明テロップを撤去。
- カードが手札へ加わるドロー演出は維持し、操作を伴う「戦線整理」の引き直し案内だけを残すよう調整。

## 1.35.11 — 2026-09-05

- カード上のステータス変動中に、四隅のLIFE・ATK・DEFバッジが絶対配置から外れて移動する不具合を修正。
- バッジの中心位置を固定したまま、数値カウント・赤青の発光・約6%の控えめな拡大のみ行うよう調整。

## 1.35.10 — 2026-09-05

- ステータス変動時のLIFE・ATK・DEFバッジを固定し、数値カウントと赤青の発光だけで変化を表現。
- 数値変化を覆っていた「LIFE 10 → 15」等のフローティングテロップを撤去。

## 1.35.9 — 2026-09-05

- カード・修行・技・ターン開始効果によるLIFE・ATK・DEF・TPの増減を、旧値から新値へのカウント変化と対象数値の発光で表示。
- 能力上昇は赤、低下は青の演出に統一し、カード使用演出や攻撃演出の解決後に数値変化が始まるよう順序を調整。
- TP前借りと最大TP増減をターン交代演出後に表示し、通常10個・11以上は赤橙1個を2TPとして表す固定10枠のTP宝石へ刷新。

## 1.35.8 — 2026-09-05

- 全5段階のAIで、同じモンスターが非致死攻撃を行う前にATKトレーニング・攻撃修行を使える場合は、育成を先に行うよう行動順を補正。
- 今すぐ勝利・敵撃破できる攻撃は育成より優先し、育成すると攻撃用TPが不足する場合も攻撃を維持する例外を追加。
- アリーナD〜MASTERの公式AIについて、デッキ強度と行動AIの対応関係を回帰テストで固定。

## 1.35.7 — 2026-09-05

- 遊び方に、墓地から山札へ戻ったカードも大会中の成長値と習得技を保持することを追記。
- 再召喚時は残りLIFE、一時効果、合体・覚醒状態をリセットする境界を明記。
- 覚醒説明を現行仕様に合わせ、後攻10ターン到達で両者に解禁され、各プレイヤー1回ずつ使用できる表記へ修正。

## 1.35.6 — 2026-09-05

- 試合BGMに追加の50%トリムを適用し、BGM設定100でも元音源の25%を最大出力とする75%オフへ変更。
- 後攻10ターン到達を両プレイヤー共通の覚醒解禁タイミングとし、先攻・後攻が各1回ずつ覚醒できるよう修正。
- 「戦線整理」を手札から1〜2枚直接選び、確定後に山札へ戻して同数を順番に引き直す専用操作・演出へ刷新。
- 「素材探索」で山札上5枚を公開し、複数のモンスターから1枚を選べる確認画面を追加。対象なしの場合も一時表示後に山札へ戻る演出を追加。
- 素材探索のカード使用演出を使用者の山札へ向け、効果カードの確認・選択・解決が順に見えるよう調整。

## 1.35.5 — 2026-09-05

- 提供された試合用楽曲を追加し、トーナメント・アリーナの試合中だけ再生するよう対応。
- 音量設計を固定マスター50%とBGM設定0〜100の乗算へ変更し、初期BGM設定100で実出力50%になるよう修正。
- ホームと試合のBGM設定を共通化し、独立したSE音量設定と今後の効果音再生用オーディオバスを追加。
- 旧設定の初期値50は新設計へ持ち越さず100へ移行し、明示的なミュート設定だけは維持。
- 両BGMをオンデマンド読込とし、マナーモード・画面非表示時の停止処理を共通化。

## 1.35.4 — 2026-09-05

- 提供された楽曲をホーム画面専用BGMとして追加し、タイトル画面のタップ後にオンデマンドで再生するよう対応。
- 初期音量50%、0〜100の1刻みスライダー、ミュート操作、端末内への音量設定保存を追加。
- iPhoneのマナーモードに従うambient音声経路を採用し、利用できないiOS環境では安全のため消音するフォールバックを追加。
- ホーム画面以外・アプリ非表示時にBGMを停止し、ホームへ戻るかアプリを再表示した際は同じ位置から再生するよう対応。

## 1.35.3 — 2026-09-05

- 終盤の高ATKモンスターへの対抗札として、攻撃技を1ターン封じる「封印の鎖」を追加。
- 敵味方を問わず最高ATKの1体を墓地へ送る「粛清」を追加し、同率時は対象を選択可能にした。
- 次の相手ターン中に指定した味方を破壊した敵も墓地へ送る「道連れの契約」を追加。
- 3枚の固有イラストと軽量サムネイル、状態表示、CPUデッキ選出・評価、期限処理の自動テストを追加。

## 1.35.2 — 2026-09-05

- 特別イラストのモンスターカードを常にFoil加工として生成・表示し、既存の通常加工データも読み込み時にFoilへ移行。
- 未所属カード一覧で長いカード名がカード幅を押し広げないよう固定し、クロノギアだけ大きく見える表示崩れを修正。
- ブースター開封前は袋だけでなく画面内のどこをタップしても開封演出を開始できるよう変更。

## 1.35.1 — 2026-09-05

### Special-fusion field hint
- When a monster card is selected after fusion unlock, distinguish compatible special-fusion mains on the player field with a cyan, violet, and gold animated aura.
- Mark the matching card as `SPECIAL` and change its compact SP preview to `特殊合体`, while leaving normal-fusion targets on the existing standard highlight.
- Derive the hint only from currently legal special-fusion actions, so it respects turn unlocks, fusion locks, TP costs, and unit eligibility.

## 1.35.0 — 2026-09-05

### Title screen
- Insert a dedicated title screen between startup loading and the home lobby, with an explicit tap gesture ready for future BGM playback.
- Add newly generated native 16:9 title artwork representing all six monster classifications without reusing or stretching card art.
- Add a slow `TAP TO START` pulse, keyboard-accessible start control, reduced-motion support, and critical-image preload.

## 1.34.1 — 2026-09-05

### Normal-draw rate disclosure
- Replace overlapping “at least once per pack” card probabilities with the average composition of the three normal draw slots, excluding the guaranteed monster and Rare-or-better slots.
- Make every regular card table total exactly 100% and label the figures as normal-draw composition rather than per-pack acquisition chances.
- Separate special-art rates into the next-pack appearance chance and a conditional five-card illustration breakdown that totals 100%.

## 1.34.0 — 2026-09-05

### Monthly missions and monster exchange tickets
- Add six month-long objectives derived from the existing login, battle, arena, and tournament mission actions.
- Award one monster card exchange ticket after all monthly objectives are complete, with idempotent claim and exchange persistence.
- Add a Shop exchange flow for choosing any base monster in normal or special artwork and normal or Foil finish.

### Clearer booster disclosure
- Explain all displayed rates as the probability of obtaining at least one matching card in the next five-card pack.
- Remove the internal draw-slot column from the player-facing card table.
- Collapse overlapping guarantees to the strongest applicable promise so special-art guarantees do not look like several guaranteed cards.

## 1.33.8 — 2026-09-05

### Daily mission claim isolation
- Restrict mission reward claims to claimed-ID updates while preserving login, battle, and win counters exactly as they were before the claim.
- Add a regression test proving that claiming the daily login reward cannot complete or unlock the daily battle and victory rewards.

## 1.33.7 — 2026-09-04

### Continuous support-card travel
- Remove the pause after effect-copy fade so the card immediately flows into its target.
- Release and fade the cinematic radiance during travel instead of leaving the mandala-like background visible.

## 1.33.6 — 2026-09-04

### Support-card copy fade sequencing
- Release the completed copy entrance animation before starting its fade so the effect description can actually disappear.
- Wait for the opacity transition to finish, then guarantee the copy is hidden before the card pauses and travels into its target.

## 1.33.5 — 2026-09-04

### Support-card copy timing correction
- Restore the original readable-copy duration for Training, shugyo, and breeder cards.
- Move the added delay entirely after the explanation has faded, keeping the card still before absorption without prolonging its text.

## 1.33.4 — 2026-09-04

### Support-card cinematic timing
- Hold Training, shugyo, and breeder effect copy on screen after its entrance so the card name and explanation can be read before clearing.
- After the copy has fully faded, keep the card still for a distinct beat before it travels into its target monster, board, hand, or player field.
- Preserve the same phase order for player and CPU actions while retaining proportionally shorter fast and reduced-motion timings.

## 1.33.3 — 2026-09-04

### Brighter home artwork
- Raise the home illustration's luminance and saturation while preserving its contrast.
- Reduce the dark edge, top/bottom, radial, and inset overlays so both the leader and environment remain visible behind the readable UI panels.

## 1.33.2 — 2026-09-04

### My Page battle-record spacing
- Cap the dashboard's top row so champion and signed-in account copy can no longer consume the battle-record height.
- Keep profile status copy on one ellipsized line and reserve the remaining lower row for readable battle metrics.

## 1.33.1 — 2026-09-04

### My Page layout renewal
- Keep profile, account recovery, and catalog progress in the always-visible top row so a new account can immediately reach registration and login.
- Move home artwork and a narrower two-column battle record into the lower row, preventing oversized statistics cards from pushing account controls out of view.
- Fit the dashboard to one landscape screen with a compact-height variant instead of depending on a hidden vertical scroll.

## 1.33.0 — 2026-09-04

### Mobile home-screen icon renewal
- Replace the plain MC initials with an original mechanical-monster crest combining a magical card frame, dark forged armor, antique gold, and a luminous cyan core.
- Generate dedicated 180, 192, 512, and maskable 512/1024 PNGs from one high-resolution source; keep the source out of the deployed Pages artifact.
- Use the matching raster icon for the browser favicon so the installed game and site share one identity.

## 1.32.4 — 2026-09-04

### Support-card cinematic timing
- Wait for the card name and effect description to finish fading out before Training, shugyo, and breeder cards begin travelling into their target.
- Preserve the same presentation order for players and CPU opponents at both normal and fast battle speeds.

## 1.32.3 — 2026-09-04

### Arena representative thumbnail
- Render the representative monster in ranking rows with the lightweight catalog thumbnail assets instead of an unstyled card portrait.

## 1.32.2 — 2026-09-04

### Resilient Arena ranking reads
- Calculate the current player's position and nearby rows directly from TOP 50 whenever possible, avoiding the failing secondary query for ranked players.
- Keep the available TOP 50 visible even if total-count or out-of-range nearby aggregation temporarily fails.

## 1.32.1 — 2026-09-04

### Arena leaderboard retry
- Do not cache failed Arena leaderboard requests, so reopening the ranking immediately retries after a temporary Firestore index or network failure.
- Replace the misleading online-only message with a retry instruction.

## 1.32.0 — 2026-09-04

### Arena leaderboard and artwork focus
- Add a live Arena rating leaderboard with TOP 50 and nearby-player views, current position, icons, ranks, records, and representative monsters.
- Publish one privacy-limited ranking profile per Arena player and keep it synchronized after battles.
- Turn the centered MC mark into a hidden full-screen home artwork viewer that closes with a single tap.

## 1.31.5 — 2026-09-03

### Brighter home artwork
- Raise the home artwork brightness and saturation while keeping lighter edge shading for menu readability.

## 1.31.4 — 2026-09-03

### Home artwork picker
- Keep monster names and artwork variants fully visible in compact iPhone landscape layouts by preventing grid rows from collapsing.

## 1.31.3 — 2026-09-03

### Reliable startup progress
- 起動画面へ処理段階に連動するプログレスバー、進捗パーセント、現在の読み込み内容を追加した。
- クラウド初期化・起動時同期が10秒を超えた場合は端末内バックアップで起動を継続し、通信待ちで画面が止まり続けないようにした。
- ホーム画面の全画面イラストを非同期デコードへ変更し、iPhoneで画面回転後まで初回描画が保留される可能性を除いた。

## 1.31.2 — 2026-09-03

### Turn draw presentation
- 自分の通常ドローをターン交代演出の完了後に開始し、引いたカードを山札側から手札へ1枚ずつ表示する演出を追加した。
- 通常速度、高速、視差効果を減らす設定にそれぞれ専用時間を設け、演出中は手札・山札枚数も表示進行に同期する。

## 1.31.1 — 2026-09-03

### Staggered battle fields
- 自分フィールドを基準位置に保ったまま相手フィールドを右へ半枠弱ずらし、上下のカードが一直線のレーンに見えない交互配置へ変更した。
- 空き枠、選択判定、対象ハイライト、カード使用演出は従来のスロットIDを維持したまま表示座標へ追従する。

## 1.31.0 — 2026-09-03

### Battle card-use cinematic
- ブリーダー、Training、修行カードの使用時に、使用カード名と効果文を拡大表示する共通演出を追加した。
- 対象指定カードは対象モンスターへ縮小・収束し、対象なしカードは手札、自陣、相手陣、プレイヤーHUDのうち効果に対応する場所へ着地するようにした。
- TP回復、ドロー、能力上昇、防御、妨害などを色・紋章・結果ラベルで区別し、CPUが使用したカードも `ENEMY CARD` として公開するようにした。
- プレイヤーとCPUで演出時間を完全に共通化し、標準約1.02秒、高速約0.34秒、視差軽減約0.28秒とした。
- バトルエンジンには表示や待機処理を追加せず、確定行動からUI演出モデルを生成する構成として将来のPvP流用性を維持した。

## 1.30.0 — 2026-09-03

### Home artwork selection
- ホーム画面イラストをデッキリーダーから独立させ、マイページから直接選択できるようにした。
- 初回更新時は、それまで表示していた先頭デッキのリーダーと通常絵・特別絵・Foil状態を引き継ぐ。
- 選択内容を端末保存とFirebaseアカウントの双方へ保存し、デッキを変更してもホームイラストが変わらないようにした。
- 全30体の特別絵に、カード絵とは異なる姿勢・視点で描いたホーム専用16:9イラストを追加した。
- 30枚を合計約4.32MBのWebPへ最適化し、選択中の1枚だけをオンデマンド読込するようにした。
- 選択一覧は通常絵・特別絵60候補をまとめた約151KBのサムネイルアトラス1枚だけを読み込むようにした。

## 1.29.0 — 2026-09-03

### Special illustrations and missions
- monster-001〜024の特別絵を、通常絵とは異なる姿勢・視点・シルエットの新規イラストへ差し替えた。
- 特別絵は640×853のWebPへ最適化し、対象24枚の合計容量を従来より軽量化した。
- 達成済みミッションの「受け取る」ボタンを明るい金色と発光表現に変更し、受取可能状態を一目で判別できるようにした。

## 1.28.2 — 2026-09-03

### Home lobby
- iPhone横向きのセーフエリアがある場合も、バージョン表記をショップボタンと重ならない画面下部へ移動した。

## 1.28.1 — 2026-09-03

### Card behavior audit
- 「素材探索」の確認対象5枚にモンスターがない場合でも使用でき、確認したカードを山札の下へ送って正常に解決するよう修正した。
- 空振りになる「素材探索」はCPUが無駄に使用せず、カードとTPを温存するようにした。
- 全30体の召喚、全5枚のTraining・修行、全270技、全52枚のブリーダーカードを総当たりする使用可能性監査を追加した。
- 初期ブリーダー20枚の主要な効果値・対象条件・付与状態をカード記載と照合する回帰テストを追加した。

## 1.28.0 — 2026-09-03

### Awakening validation and cinematic
- 覚醒率、後攻10ターン目の覚醒率、覚醒同ターン勝利率、決着能力を集計する再現可能なAI監査指標とストレス試験を追加した。
- 同一デッキの即時覚醒ストレス試験2,000戦で、先攻勝率45.95%、後攻勝率49.90%、引分4.15%を確認した。
- 後攻10ターン目覚醒336件のうち同ターン勝利は10件（2.98%、全試合の0.50%）で、多発していないことを確認した。
- 通常Silver AIミラー24戦では先攻・後攻とも12勝、10ターン目覚醒13件の同ターン勝利は0件だった。
- 覚醒実行時に、素材消滅、空間亀裂、多重魔法陣、覚醒体顕現、能力名開花を描く全画面演出を追加した。
- 標準覚醒演出を3.3秒とし、特殊合体より長く派手にしつつ、高速・視差軽減モードも維持した。

## 1.27.3 — 2026-09-03

### Arena opponent selection
- 公式AIのリーダーをデッキ内の先頭カードではなく、デッキ分類に一致するモンスターから選ぶよう修正した。
- 候補更新時の分類に幅を持たせ、更新を重ねることで6分類すべてが候補に出現するよう改善した。
- CPU操作に関する補足文を削除し、候補更新ボタンを見出し右側へ移動した。
- 対戦相手選択画面の外側余白・高さ・カードサイズを調整し、iPhone横向きでもスクロールや重なりなしで1画面に収めた。

## 1.27.2 — 2026-09-02

### Missions and Arena
- ミッション画面から戦利品ストックを撤去し、アリーナ画面ヘッダーの旧ミッションボタン位置へ移動した。
- アリーナ画面のミッションボタンを廃止した。
- ミッション報酬の受取ボタンがグリッド幅を超えて崩れないよう修正した。

## 1.27.1 — 2026-09-02

### Arena opponent selection
- 格下・同格・格上の公式AI候補に異なるレートを設定し、強さの区分がレート表示にも反映されるようにした。
- 対戦相手選択時は自身のレート・勝敗パネルと未受取報酬を隠し、選択内容が1画面に収まる専用レイアウトにした。
- 対戦相手選択画面に、ホームとは別に使用デッキ選択へ戻る「戻る」ボタンを追加した。

## 1.27.0 — 2026-09-02

### Arena opponent selection
- 対戦相手の検索後は使用デッキの再選択を表示せず、確定済みのデッキを引き継ぐように変更した。
- 格下・同格・格上の3候補を同時に提示し、プレイヤーが対戦相手を選べるようにした。

## 1.26.2 — 2026-09-02

### Home lobby
- ホーム画面右下のバージョン表示を画面基準で固定し、ナビゲーションより前面に配置して、背景上でも読み取れるようにした。

## 1.26.1 — 2026-09-02

### Arena
- 「使用するデッキ」のリーダーカードをトーナメントと同じイラストのみの表示に変更し、コスト・LIFE・TP・ATK・DEFを非表示にした。

## 1.26.0 — 2026-09-01

### Rating Arena
- Added a one-match rating arena with D through MASTER ranks and resumable battles.
- Added 36 honest official opponents across six classifications and six ranks.
- Added player defense deck registration, recent-opponent avoidance, and deck-composition similarity weighting.
- Added MASTER-only `LEGEND ARCHIVE` matchmaking backed by champion snapshots recorded from this release onward.
- Added weekly arena loot stock and one-card acquisition after three wins.
- Added claimable rank-up rewards, diamonds, pack credits, titles, and persistent arena records.

### Missions
- Added daily login, battle, and victory missions plus weekly arena and tournament missions.
- Added the home mission button between Gift and Notices, with a red badge only for completed unclaimed missions.
- Kept the existing automatic daily 300-diamond login award and marked that mission as received to prevent double rewards.

## 1.25.2 — 2026-09-01

### Home lobby alignment
- Aligned the gift, notice, and help rail to the same right edge as the diamond wallet.
- Preserved the iPhone landscape safe area instead of overriding it with a fixed 12px inset.

## 1.25.1 — 2026-09-01

### Added

- 全30体のリーダーへ、既存の縦長カード絵を拡大したものではない、ホーム画面専用の横長一枚絵を追加した。

### Changed

- ホーム背景を専用アートへ切り替え、モンスター全身と周囲の情景が見える横長構図に統一した。
- ホーム専用アートは現在のリーダー1枚だけをオンデマンド取得し、全30枚をPWA初回インストール時に一括取得しないよう軽量化した。

### Verified

- 30枚すべてのWebP形式、ファイル容量、参照先、および初回プリキャッシュ除外を自動テストで確認した。

## 1.25.0 — 2026-09-01

### Added

- 9月4日まで受け取れる「ホーム画面刷新記念」ギフトを追加。ギフトボックスから1アカウント1回、ダイヤ3,000個を受け取れる。
- マイページのプレイヤーアイコンをタップし、所持したことのあるカードの軽量サムネイルからアイコンを選択できる機能を追加。設定は端末と復旧アカウントの両方へ保存する。
- ホーム画面のコレクションレベル直下へ、現在の進捗が分かる常時表示のプログレスバーを追加。

### Changed

- 夏休みボーナスの配布期間を8月31日で終了し、9月以降はデイリーログインボーナスのみ自動付与するよう変更した。
- 通常プレイ時のクラウド同期警告帯をホーム画面から削除し、下部メニューを画面中央へ固定した。
- ホーム用リーダーイラストをわずかに縮小し、実機横画面で全体像が見えやすくなるよう調整した。

### Verified

- 期間境界、ギフトの一度限りの受取、ローカル・Firebase双方の同時更新、プレイヤーアイコンの保存と復元を自動テストで確認した。

## 1.24.0 — 2026-09-01

### Changed

- ホーム画面を、選択中デッキのリーダーイラストを全面に見せるロビー型UIへ刷新。画面中央のモンスター名表示を省き、イラストそのものを主役にした。
- 左上にユーザー情報と図鑑進捗、左側に現チャンピオンと大会導線、右上に所持ダイヤ、右側にギフト・お知らせ・遊び方を集約した。
- 下部ナビゲーションをトーナメント・アリーナ・ホーム・カード・ショップの5項目へ再編し、各名称が省略されにくい間隔を確保した。アリーナは将来実装予定として表示する。
- リーダーの通常絵・特別絵・Foilをホーム画面にも反映し、表示中の1枚だけを遅延なく読み込む構成にした。

### Verified

- 大会の新規開始と再開、保存デッキ、ブースター、マイページ、遊び方、管理者ツール、PWA追加の既存導線を新ホームから利用できることを確認した。

## 1.23.0 — 2026-09-01

### Added

- 後攻10ターン目から1試合に1回使える「覚醒」を実装。未行動の場のモンスターを選び、召喚酔いしていない別の味方1体を撃破扱いにせず墓地へ送ることで、LIFE・ATK・DEFを各15上昇させる。
- 基本モンスター30体と特殊合体60体の全90体へ、固有の覚醒能力・効果説明・発動上限を追加。既存特性を維持しつつ、回復、貫通、軽減、連撃、撃破報酬など各個体の役割を伸ばす。
- 覚醒解禁時に、専用魔法陣・回転光輪・粒子・全画面タイトルを組み合わせた演出を追加。通常、高速、視差軽減の各設定に対応した。
- バトル詳細の技一覧直下へ覚醒ボタンと能力プレビューを追加し、素材候補の状態を確認して選べる専用モーダルを実装。覚醒後はカード配色、紋章、状態表示、詳細能力欄へ反映する。

### Changed

- 覚醒済みモンスターは以後合体できず、場を離れても同じ試合では覚醒を再使用できない仕様に統一した。
- AIの全難易度が覚醒を合法行動として評価し、素材損失と能力上昇を盤面評価・先読みへ反映するようにした。
- 遊び方と初心者チュートリアルへ、覚醒ターン、素材条件、能力上昇、使用回数を追加した。

### Verified

- 全30基本モンスター・全60特殊合体に覚醒能力が一意に存在すること、後攻限定・10ターン目・1試合1回・召喚酔い素材禁止・墓地送り・能力上昇・合体禁止を自動テストで確認した。
- 覚醒状態を含む戦闘チェックポイントの後方互換を維持し、従来の戦闘・AI・大会・永続化・PWAテストに回帰がないことを確認した。

## 1.22.2 — 2026-09-01

### Added

- 新ブースター限定モンスター6体へ、各2種・計12種の特殊合体を追加。素材は新6体以外かつメインと異なる分類から重複なく選定し、特殊特性も各完成形専用で実装した。
- `fusion-049`〜`fusion-060`へ、メインの造形を約80%維持して素材の特徴を装備・紋様へ統合した通常絵と、同じ完成形を別構図・別姿勢で描いたFoil特別絵を追加した。
- 図鑑用の独立サムネイル12枚と、第3世代特殊合体の生成記録・回帰テストを追加した。

### Changed

- 特殊合体の収録数、図鑑、管理者ツール、仕様書の基準件数を48種から60種へ更新した。

### Verified

- 12レシピすべてについて、メインごと2種、素材分類違い、旧24体からの選定、素材重複なしを自動検証した。
- 通常絵・特別絵を640×853の高品質WebP、図鑑用サムネイルを180×240の軽量WebPへ最適化し、一覧で主役逆転・多重人物・手足破綻がないことを確認した。

## 1.22.1 — 2026-08-31

### Added

- 新ブースター限定モンスター6体に、通常絵と異なる姿勢・カメラ・背景で描いた奪取不可の特別イラストを追加。全画像を640×853の高品質WebPへ最適化した。
- 各分類の特別イラスト候補を5種へ拡張し、管理者ツールでも全30種を確認可能にした。

### Changed

- 初回および5パックごとの保証対象を、各分類のクロノギア世代と最新世代の2体から均等抽選する方式へ変更。全体で`monster-019`〜`monster-030`の12体が対象。
- 提供割合画面とブースター説明を新しい2体保証・5種特別絵へ同期。通常時の特別絵は各0.8%、20パック目は各20%。

### Verified

- レオネアの初回特殊絵候補は腕の本数が曖昧だったため破棄し、明確な二腕構図で再生成して採用。
- 保証抽選、提供割合、全30特殊絵ファイル、管理者一覧、ブースター画面の自動テストに合格。

## 1.21.10 — 2026-08-31

### Changed

- 元作品を強く連想させる基本モンスター8体の44技を、本作独自の技名へ変更。技ID、Rank、威力、TP、追加効果、初期習得、修行区分は維持した。
- 条件付き威力、TP軽減、DEF無視、1試合1回制限の判定を表示名依存から安定した`move-XXX`基準へ変更した。
- 修行プールと管理者・図鑑・保存デッキ・バトル詳細を新名称へ統一し、ドラゴン側の同名技`move-051`は変更対象外として維持した。
- 旧162技ワークブックから現在の216技マスターを誤って縮退再生成しない安全確認を追加した。

### Verified

- 844×390の横画面で対象8体の全技詳細を確認し、ダイアログの横溢れおよび技名の切れがないことを確認。
- 全228件の自動テストとv1.21.10 GitHub Pages/PWAビルド（264ファイル、24,892,129 bytes）が成功。

## 1.21.9 — 2026-08-31

### Changed

- 特殊合体48体のうち、元作品固有の派生名や旧基本名を含む25体を本作独自名称へ変更。オキクサンを含む一般語・伝承由来・第2世代特殊合体の23体は維持した。
- `fusion-001`〜`048`、通常絵・特別絵の画像パス、合体レシピ、固有特性、発見履歴は変更せず、表示名と名称参照処理だけを移行した。
- 旧バトルチェックポイント内の特殊合体名は、`specialFusionId`を優先して新名称へ復元し、IDがない旧データも旧名対応表から移行する。
- 覚醒は引き続き保留し、仕様・挙動を変更していない。

### Verified

- 844×390のiPhone横向き相当で特殊合体48体を表示し、最長名称の2行表示、通常絵、特別絵、カード枠に欠けがないことを確認。
- 全224件の自動テストとv1.21.9 GitHub Pages/PWAビルド（263ファイル、24,890,887 bytes）が成功。

## 1.21.8 — 2026-08-31

### Added

- 先攻は初手3枚から最大3枚、後攻は初手5枚から最大5枚を交換できるマリガンを追加。交換札の即時引き直しを防ぎ、両者確定後にターン1を開始する。
- CPU用の非チートな自動マリガン方針と、マリガン途中を含むバトルチェックポイント保存を追加。

### Changed

- 奪取限定カードの勝利後候補抽選重みを通常カードの5倍へ変更。40枚中1枚なら候補出現率は約46.98%、2枚ならどちらかが約70.69%。
- 基本モンスター7体を、ギアセンチネル、アストラノイド、アルカナロード、ルミラビ、ボルトウルフ、コンゴウ、フェザーレックスへ改名。安定IDを維持し、カード、技、合体レシピ、修行、AI、図鑑、旧バトル復元へ反映。
- 覚醒は仕様検討のため保留し、ゲーム挙動を変更していない。

### Verified

- 844×390のiPhone横向き相当で後攻5枚を一画面表示し、カード選択、交換確定、先攻CPUの3枚交換、ターン開始まで確認。
- 全223件の自動テストとv1.21.8 GitHub Pages/PWAビルド（262ファイル、24,888,425 bytes）が成功。

## 1.21.7 — 2026-08-31

### Added

- 16人トーナメント画面へ、王冠・観客席・儀礼旗を備えた専用のダークファンタジー大会背景を追加。1599×900のWebP（203,356 bytes）へ軽量化し、中央の対戦表が読める暗部を確保した。
- 大会開始時に背景光、見出し、対戦表、案内の短い入場演出を追加。`prefers-reduced-motion`ではすべて停止する。
- 攻撃命中時の短い衝撃リングと、Training／修行時の簡易魔法陣演出を追加。演出だけの変更で、ダメージ計算・行動時間・AI判断には影響しない。
- マリガン、奪取限定カードの提示率、覚醒、名称変更の検討材料を`DESIGN_ROADMAP_2026-08-31.md`へ記録した。合意前のルール変更は実装していない。

### Verified

- 844×390の大会画面で専用背景、対戦表の可読性、横方向のオーバーフローなしを確認。実バトルでCPU攻撃時の命中演出を確認し、ブラウザーwarning/errorは0件。
- 全215件の自動テストとv1.21.7 GitHub Pages/PWAビルド（260ファイル、24,870,602 bytes）が成功。

## 1.21.6 — 2026-08-31

### Fixed

- 縦向き警告の背後でゲーム本体と図鑑129枚が再レイアウトされ続ける問題を修正。縦向き中は`.app-shell`と`#modal-root`を描画対象から外し、警告だけを固定表示する。
- `body`を縦向き中だけ固定し、スクロール・overscroll・touch操作と警告のanimation/transitionを停止。iPhone Safariの可変表示領域による揺れを抑止。
- 縦向き警告の幅制限を撤廃し、760pxを超える縦向き端末でも横向き案内を表示。

### Verified

- 844×390の図鑑129件表示から390×844へ回転し、警告の矩形が5回連続で一致、図鑑Gridの描画矩形が0件になることを確認。
- 横向き復帰は約106msで完了し、図鑑Gridとカードの矩形が回転前と一致。768×1024の縦向きでも警告を確認。
- 全213件の自動テストとv1.21.6 GitHub Pages/PWAビルド（259ファイル、24,662,867 bytes）が成功。

## 1.21.5 — 2026-08-30

### Fixed

- 図鑑の基本カードサムネイル画像領域をカード名帯の直下から開始し、モンスターやブリーダーの顔が帯に隠れない配置へ変更。
- 軽量WebPアトラスを再圧縮せず、セルの縦横比を保ったまま下端側をトリミングするため、既存絵の画質と構図を維持。
- ホームのアプリ版表示を絶対配置から専用の最下段Grid行へ移し、`遊び方`ボタンの内側ではなく、その下に表示。

### Verified

- 1280×568で図鑑の基本カードを確認し、カード名帯と画像表示領域が分離し、モンスター・Training・修行の主要被写体が読めることを確認。
- 同画面で`v1.21.5`の上端がホーム操作欄の下端より下にあることを座標計測とスクリーンショットで確認。
- 全213件の自動テストとv1.21.5 GitHub Pages/PWAビルド（259ファイル、24,662,269 bytes）が成功。

## 1.21.4 — 2026-08-30

### Fixed

- 既存PWAを開いた際、旧Service Workerが`?v=`を無視して前版の画面モジュールを返す更新停滞を修正。版付きリクエストは完全一致キャッシュとし、オンラインでは必ず新版を取得する。
- 新しい`index.html`自身から版付きService Workerを先行登録し、旧画面モジュールが残っていても新Workerへ更新できる経路を追加。切替時は一度だけ自動再読込する。
- オフライン時は同じWorkerが事前保存した版なしファイルへフォールバックし、更新の確実性とオフライン起動を両立。

### Verified

- 全213件の自動テストとv1.21.4 GitHub Pages/PWAビルド（259ファイル、24,662,097 bytes）が成功。

## 1.21.3 — 2026-08-30

### Fixed

- iPhone Safariで図鑑カードが再び細い帯へ潰れる問題に対し、`aspect-ratio`による行高の逆算を廃止。画面幅別の明示的なGrid行高を持たせ、基本カード・特殊合体・所持状態を問わず同じ高さへ固定。
- クロノギア以前の旧モンスター詳細を縦長4:3、ブリーダー・修行・Training詳細を正方形として、幅と高さを個別に指定。Safariでもイラストが縦方向へ圧縮されない表示へ変更。
- ホームの大会ボタンに行間と個別line-heightを設定し、`大会へ挑戦`と出場可能杯の説明が重ならないよう修正。
- ホーム最下部へ現在のアプリ版`v1.21.3`を常時小さく表示。PWAキャッシュ版と同じ値であることを自動テストするよう追加。

### Verified

- 844×390のiPhone横向き相当で全6フィルターを切り替え、各カードが高さ104.65pxで一致することを確認。旧モンスター詳細は縦横比1.333、Training詳細は1.000を維持。
- 同画面でホームの大会3行が互いに重ならず、最下部版表示が見えることを確認。ブラウザーwarning/errorは0件。
- 全213件の自動テストとv1.21.3 GitHub Pages/PWAビルド（259ファイル、24,661,051 bytes）が成功。

## 1.21.2 — 2026-08-30

### Fixed

- iPhone Safari／ホーム画面PWAで、図鑑カードの高さが細い帯へ潰れたり、行間に大きな空白が生じる表示崩れを修正。
- 図鑑カード自身へ確定した縦横比を持たせ、内部カードを親領域へ絶対配置。SafariのGrid・描画分離・子要素aspect-ratioの組み合わせに依存しない構造へ変更。
- 軽量アトラス導入後は不要になった図鑑項目の`contain: layout paint`を撤廃し、全フィルターで同じ行高を維持。

### Verified

- 1280×568で全件、所持済み、未所持、モンスター、育成・ブリーダー、特殊合体を順に切り替え、各フィルターの先頭12枚が高さ約140px・縦横比1.3888で一致することを確認。
- 全212件の自動テストとv1.21.2 GitHub Pages/PWAビルド（258ファイル、24,660,321 bytes）が成功。ブラウザーwarning/errorは0件。

## 1.21.1 — 2026-08-30

### Changed

- カード図鑑の全129件を、基本カード用・特殊合体用の2枚の軽量WebPサムネイルアトラスで描画。詳細を開いた時だけ元の高解像度画像を読み込む構成へ変更。
- 特殊合体と単体画像カードの詳細を背景切り抜き表示から実画像の全体表示へ変更し、元画像の縦横比を維持して上下左右の欠けと拡大ボケを抑制。
- ホームの大会再開ボタンを1行省略表示へ固定し、狭い横画面での文字はみ出しを防止。チャンピオンのリーダー画像を最大185×247pxへ拡大。
- 復旧設定済みのマイページ表示を`ACCOUNT / 〇〇でログイン中`へ簡略化。

### Performance

- 81基本カードを273,080 bytes、48特殊合体を200,686 bytesの2アトラスへ集約。図鑑表示時の多数の原画リクエストと`content-visibility`由来の初期配置遅延を解消。
- 図鑑サムネイルは初回PWAインストール時の事前キャッシュから外し、図鑑を初めて開いた時だけ取得する。

### Verified

- 1280×568の実ブラウザーで、ホーム再開表示の横溢れなし、リーダー画像185×247px、図鑑先頭行の即時表示、特殊合体原画の全体表示を確認。
- 全211件の自動テストとv1.21.1 GitHub Pages/PWAビルド（258ファイル、24,660,288 bytes）が成功。

## 1.20.1 — 2026-08-30

- マイページの未登録アカウント欄を`ACCOUNT / 新規登録 / ログイン`の3要素へ簡略化し、スマホ横画面でも戦績と図鑑進捗が画面内に収まるよう再配置。
- Firebase Authenticationの永続ログイン復元完了を待ってから匿名認証の要否を判定。低速なPWA起動時に既存ユーザーを新規匿名ユーザーとして扱う競合を防止。
- 844×390の実ブラウザー確認で本文の縦横オーバーフローなし、アカウント欄の全操作表示を確認。
- 全210件の自動テストとv1.20.1 GitHub Pages/PWAビルド（256ファイル、24,182,363 bytes）が成功。

## 1.20.0 — 2026-08-30

### Added

- ホーム右上の名前変更を「マイページ」へ置き換え、プレイヤー名変更、アカウント保護、戦績、カード図鑑進捗を1画面へ集約。
- 現在のFirebase匿名UIDへ任意の復旧ID／パスワードをリンクする復旧設定と、新端末での既存アカウント復旧を追加。メールアドレスはユーザーに要求せず、IDを内部専用の非配送疑似メールへ決定的に変換する。
- 通算試合、勝敗、勝率、連勝、大会参加／優勝、杯別優勝、王座獲得、確定奪取枚数をoperation ID付きで記録する戦績モデルを追加。
- 初回と毎ターン交代時に、青白の`YOUR TURN`／赤の`ENEMY TURN`を多重回転魔法陣と現在ターン数で示す全画面演出を追加。標準・高速・動きの軽減設定に対応。

### Safety

- 匿名端末キャッシュと復旧後アカウントのローカルバックアップをUID別スコープへ分離し、別アカウントのデッキ・図鑑・ダイヤが混在しないようにした。
- 進行中大会がある場合は別アカウントへの切替を禁止。現在の匿名アカウントへ復旧設定を追加する操作はUID不変のため許可。
- 大会再開や通信再送による戦績の二重計上を、直近320件の処理済みoperation IDとFirestore transactionで防止。

### Setup

- `FIREBASE_SETUP.md`へ内部で利用するEmail/Passwordプロバイダー、復旧ID変換、匿名アカウント自動クリーンアップの注意を追記。

### Verified

- 全209件の自動テストとv1.20.0 GitHub Pages/PWAビルド（256ファイル、24,181,818 bytes）が成功。
- 844×390と667×375でホーム、マイページ、復旧登録フォームを確認。横溢れ・縦スクロールなし、ブラウザーwarning/errorは0件。

## 1.19.0 — 2026-08-30

### Added

- デッキ編集の「入替候補」へ、使用中40枚とは独立した種類・TP昇降・名前・登録順ソートを追加。
- カード図鑑へ全81基本カードと全48特殊合体を常時掲載し、未所持・未発見は鍵表示とグレーアウト、専用フィルターで収集状態を確認可能にした。
- 画像寸法・フォルダー別容量を再計測できる`audit-images.py`と、実表示向けWebPを再生成する`optimize-card-images.py`を追加。

### Changed

- デッキ一覧、リーダー選択、大会エントリー、チャンピオン表示で、同一リーダーの特別絵＋Foil、特別絵、Foil、通常絵を一貫した優先順で解決。
- ホームのチャンピオン代表をカード枠ではなく、名前帯・四隅数値のない全体イラストとして表示。Foil外観も同じ表示へ反映。
- 通常絵のレアリティを抽選枠依存からカード固有へ変更。モンスターはRare、Training・修行・ブリーダーはCommon、特別絵はShowcaseへ統一し、旧保存資産も数量を失わず読み込み時に正規化。
- 旧特殊合体35枚を可変高さアトラスの正しい行境界から再切り出しし、縦横比を保った横512px・輪郭補正版へ更新。ブルードリル専用絵と第2世代12枚は個別原画を維持。

### Performance

- 特別絵、特殊合体特別絵、第2世代モンスター、拡張ブリーダーを、スマホ詳細の2倍密度を満たす最大640×853・WebP品質86中心へ再最適化。
- WebP合計を43.83MiBから22.72MiBへ削減。カード図鑑、管理者一覧、未所属カード、デッキ詳細／編集は画面近傍の単体絵だけを遅延読み込みする。
- 再構築用アトラスと旧ブルードリル原本はGitへ保持しつつPages配信から除外し、成果物を47,341,952 bytesから24,136,102 bytesへ削減。

### Verified

- 全194件の自動テストとv1.19.0 GitHub Pages/PWAビルド（251ファイル、24,136,102 bytes）が成功。
- 844×390でホーム、全129件図鑑、特殊合体詳細、デッキ編集を確認。横溢れなし、図鑑初期表示で単体絵は画面近傍5枚だけ読込、最終PWA新規タブのwarning/errorは0件。

## 1.18.2 — 2026-08-30

### Changed

- 奪取候補5枚が横幅を超える場合は安全に先頭から横スクロールでき、余裕がある場合は従来どおり中央配置になるよう修正。
- デッキ編集カードの長押し時に、iOS系ブラウザーの文字範囲選択とタッチコールアウトが出ないようカード内部へ限定して抑止。
- 保存デッキ一覧と詳細から、全デッキ共通で重複表示されていた「プレイヤー解禁」を削除。大会解禁ロジック自体は変更なし。

### Prepared

- カード図鑑へ未所持・未発見カードをグレーアウト表示する方針と、ホームのリーダーをカード枠なしの全体イラストで表示する方針を`NEXT_WORK_PREPARATION.md`へ追加。

### Verified

- 全187件の自動テストとv1.18.2 GitHub Pages/PWAビルド（250ファイル、47,341,952 bytes）が成功。

## 1.18.1 — 2026-08-30

### Added

- 保存デッキ詳細とデッキ編集の「使用中の40枚」へ、種類順・TPが低い順・TPが高い順・名前順・登録順の表示ソートを追加。

### Changed

- ソートはカード配列、資産ID、デッキ専用プールを変更しない表示専用処理とし、対戦開始時のseed付きシャッフルと完全に分離。
- デッキ編集中に表示順を変えても、選択中カードは`instanceId`で維持し、交換対象が別カードへずれない構造を維持。

### Verified

- 種類・TP昇順／降順・名前・登録順、元配列非変更、詳細／編集の両画面接続を自動検証。
- 全185件の自動テストとv1.18.1 GitHub Pages/PWAビルド（250ファイル、47,342,088 bytes）が成功。
- 1280×720と844×390で詳細／編集の切替を操作し、ソート後も選択中カードが維持され、横溢れとブラウザーwarning/errorがないことを確認。

## 1.18.0 — 2026-08-29

### Added

- クロノギア、アルケミア、ミストレイ、ノクティス、ヴォルファング、グラトンへ各2種、計12種の第2世代特殊合体`fusion-037`〜`048`を追加。
- 各形態へ、初回行動／初回被ダメージ、現在LIFE50%条件、撃破報酬、最大蓄積などの上限を持つ専用特殊特性を追加。
- 12形態それぞれに、メインモンスターを主役とした通常絵と、同一デザインのポーズ・カメラ・背景を変えたFoil特別絵を追加。
- 第2世代素材選定、融合デザイン、通常／特別絵の構図を追跡できる生成記録を追加。

### Changed

- 特殊合体マスター、カード図鑑、管理者一覧、描画アセット上限を36形態から48形態へ拡張。
- 通常CPUが狙う特殊合体を、従来どおり通常CPUが入手可能なモンスターだけで完成するレシピへ限定。ブースター限定の第2世代モンスターはプレイヤー、公開デッキ、現チャンピオンだけが使用する。
- 管理者一覧を基本81＋通常特殊合体48＋通常特別絵24＋特殊合体特別絵48の計201件へ更新。

### Verified

- 旧36レシピの素材採用回数を集計し、未採用のゴースト、1回採用のヒノトリ・メタルナー・ガリ・ゴーレムを優先。第2世代6体はメインとして各2レシピを持つ。
- 第2世代特性のTP下限、ターン初回判定、LIFE50%境界、復活一回性、回復、撃破報酬、蓄積上限と消費を自動検証。
- 通常絵12枚と特別絵12枚を原寸確認し、単一融合体、メイン優先、主要部位の安全余白、特別絵での同一デザイン維持を確認。
- 全182件の自動テストとv1.18.0 GitHub Pages/PWAビルド（249ファイル、47,337,534 bytes）が成功。
- 1280×720と844×390の管理者画面で全201件、通常特殊合体48件、特殊合体特別絵48件を確認。追加通常絵・Foil特別絵の詳細表示に横溢れはなく、ブラウザーwarning/errorは0件。

## 1.17.0 — 2026-08-29

### Added

- 奪取限定の上位モン類ブリーダー6枚を追加: 無機・機神起動、創造・神造再演、幻霊・黄泉の残唱、魔族・終末契約、獣族・王者の咆哮、怪物・完全捕食。
- 6枚すべてに文字・数値・カード枠を焼き込まない専用WebPイラストを追加。通常CPUの対応モン類デッキと勝利後奪取からだけ入手できる。
- 日本時間で1日1回300ダイヤのデイリーログインボーナスを追加。
- 2026年8月29日以降の初ログイン時に一度だけ3,000ダイヤを配布する夏休みボーナスと、内訳・合計・所持残高を表示する起動ポップアップを追加。

### Changed

- シルバー以上の大会解禁をデッキごとからプレイヤー全体へ変更。一度解禁した上位杯は、新規作成を含むどの保存デッキでも挑戦できる。
- 旧デッキ単位資格は全デッキ中の最高値をプレイヤー解禁へ自動移行。各デッキの最高到達実績は個別のまま維持する。
- Legend公開デッキのSecurity Rulesを、デッキ単位資格ではなくプレイヤーの`economy.tournamentQualification`と照合する形へ更新。
- ダイヤ案内を「大会報酬やログインボーナス」へ更新。

### Verified

- 新6枚の合法条件と効果、通常CPU生成、ブースター除外、勝利後の5枚提示経路を自動検証。
- 新6枚を既存ブリーダー群と並べて構図・主役・色調を比較し、類似度が高かった5枚を再生成。「魔族・終末契約」は既存の人物対悪魔構図を廃し、無人の黒曜石祭壇と日食へ差し替えた。
- 旧資格からの移行、別デッキでの上位杯エントリー、新規デッキの共通解禁継承、下位資格への逆行防止を自動検証。
- 同日再起動、翌日更新、夏休みボーナスの一回性、日本時間の日付境界、ローカル／Firestore transactionの二重受取防止を自動検証。
- 全176件の自動テストとv1.17.0 GitHub Pages/PWAビルド（224ファイル）が成功。管理者一覧177件、新カード画像、ログイン報酬ポップアップを実ブラウザーで確認し、warning/errorがないことを確認。

## 1.16.1 — 2026-08-29

### Changed

- 盤上モンスターの赤い丸を廃止し、実際に残っている強化は緑の`▲`、弱体は赤の`▼`、特殊状態は金の`◆`として分類表示するよう変更。数は各分類の有効効果数を表す。
- モンスター詳細のLIFEを「現在 / 現在の最大（残存率）」へ変更。最大LIFEはその大会で受けたTraining・修行・合体等を反映し、LIFE50%以下では条件成立を明示する。
- ジョーカーおよびフレアデスの割合条件を持つ技は、攻撃対象選択中に条件成立中の相手を発光させ、発生する威力・TP・DEF参照の変化をカード上へ表示する。
- モンスター詳細へ状態効果名と残り期間／解除条件を追加。復活済みや初回防御使用済み等の内部履歴フラグは状態数へ含めない。

### Verified

- 成長後最大LIFEを基準にした50%境界、ジョーカー／フレアデスの対象ガイド、意味のある状態だけを抽出する処理を自動検証。
- iPhone横画面相当844×390でバトル画面とモンスター詳細を確認し、横方向のページ溢れおよびブラウザwarning/errorがないことを確認。
- 全167件の自動テストとv1.16.1 GitHub Pages/PWAビルド（218ファイル）が成功。

## 1.16.0 — 2026-08-29

### Added

- 汎用対策ブリーダー6枚を追加: 強化解除指示、合体妨害工作、技術封鎖、状態浄化、反転防壁、緊急撤退指示。
- 6枚すべてに文字・数値・枠を焼き込まない専用WebPイラストを追加。全モン類ブースター、通常CPU、勝利後のカード奪取から入手できる。

### Changed

- Champion AIを最大7行動の自ターン、最大4行動の公開応手、最大3行動の既知カード継続へ拡張。技威力、状態変化、合体妨害、技TP妨害も評価し、本番思考予算を240msへ変更。
- Legend AIを最大5行動・85msへ強化。道中CPU生成は64候補、狙い特殊合体5ルート、15モンスターから選別し、1勝あたり10TP・修行最大2枚の大会内成長を行う。
- レジェンド1〜3回戦へ出す他プレイヤー公開デッキは無作為抽出ではなく品質上位を優先。プレイヤー側ブロックを中上位→上位→最上位候補が勝ち上がるシード配置にし、道中が段階的に強くなる構造へ変更。
- トーナメント形式は従来どおり16人・4試合・各ラウンド一発勝負を維持。

### Verified

- 同一40枚・左右入替24戦でChampion 17勝（70.8%）、Legend 7勝。先攻13勝、後攻11勝、40T判定0%。
- 同一40枚12戦でLegend 7勝（58.3%）、Gold 5勝。先攻／後攻は6勝ずつ。
- 35デッキ集計でLegend生成品質281.04、狙い特殊合体5.00、素材密度17.97。全対策カードの効果、CPU生成、ブースター開示、奪取候補化を自動検証。

## 1.15.17 — 2026-08-28

### Changed

- モン類ブースター一覧を、文字と操作が潰れる3列×2段から、2列の大型パックカード＋縦スクロールへ再構成。
- パック名、説明文、収録カード・提供割合、開封ボタンの文字サイズと余白を拡大。2つの操作を1行へ整理し、カード下端から開封ボタンが欠けないようにした。
- 低い横画面でも過度に縮小せず、スクロール可能な138px以上のパックカードと約41pxの操作領域を維持する。

### Verified

- 1179×544および844×390の横画面で6パック、固定フッター、縦スクロールを確認。横方向のページ溢れとブラウザwarning/errorは0件。

## 1.15.16 — 2026-08-28

### Fixed

- ホームの現チャンピオン代表画像がクロノギア等の独立画像カードでも旧アトラスを参照し、先頭のモノリスを表示していた不具合を修正。代表名と同じモンスター画像を表示する。
- 王座の40枚に保存された代表カードの外観情報をホーム描画へ渡し、通常絵だけでなく特別絵の代表カードも同じ外観で表示する。

### Verified

- クロノギア通常絵・特別絵の代表画像が独立WebPを参照し、モノリスのアトラスへフォールバックしないことを自動検証。

## 1.15.15 — 2026-08-28

### Changed

- 特別絵メインから出現する特殊合体特別絵36形態を、元カードの個別`finish`にかかわらず常にFoil表示するよう統一。盤面・手札・合体演出・詳細・管理者プレビューで同じ光沢を使用する。
- ラブラブセイジン特別絵を、破綻した逆さ姿勢から、肩・肘・手首・股関節・膝・足首を自然に追える正立の空中突進構図へ全面差し替え。銀装甲、青い目、紫結晶、曲線装飾は維持した。
- 特殊合体特別絵のバトル詳細を3:4比率へ戻し、縦長イラストを正方形に切り取らないようにした。

### Verified

- 通常仕上げの特別絵メインでも特殊合体後だけFoilになること、通常絵特殊合体・合体前の通常仕上げ特別絵・サポートカードには波及しないことを自動検証。

## 1.15.14 — 2026-08-28

### Added

- 特殊合体36形態すべてへ、通常合体絵の外見・配色・主要モチーフを保持しながら、姿勢・カメラ・動作線を刷新した特別イラストを追加。1024×1365、高品質WebPの独立画像として収録した。
- 管理者ツールへ「特殊合体特別絵」36件の専用フィルターを追加。通常特殊合体36件と並べて構図差を確認できる。

### Changed

- 特別イラストのモンスターをメインに特殊合体した場合、メインカードの外観フラグを引き継ぎ、合体演出・盤面・手札・詳細で対応する特殊合体特別絵を表示する。通常イラストをメインにした場合は従来の通常特殊合体絵を維持する。
- 15.6MBの特殊合体特別絵はPWA初回プリキャッシュから除外し、実際に表示した画像だけをランタイムキャッシュする。

### Verified

- 36画像のWebP署名・900KB未満、合体後の`artVariantId`維持、通常／特別の描画分岐、管理者一覧165件、PWA遅延配信を自動検証。

## 1.15.13 — 2026-08-28

### Added

- クロノギア以前の基本モンスター18体へ、通常絵の輪郭・顔・配色・主要モチーフを保持した特別イラストを追加。金色の円環とモン類別の光源・粒子・背景で、通常絵と一目で区別できる独立WebPにした。
- 各モン類パックの特別イラスト候補を、ブースター限定新モンスター1体＋基本モンスター3体の計4種へ拡張。
- 「収録カード・提供割合」へ特別イラスト4種を実画像で確認できる候補表を追加。通常回は各1%、20パック目は各25%を表示する。

### Changed

- 管理者ツールの特別イラスト一覧を6種から24種へ拡張。既存のパック演出プレビューでも新しい候補をseed付きで再生できる。
- 20パック保証は同モン類の4候補から均等抽選する。カード性能、通常絵、Foil／特別絵の奪取不可ルールは変更しない。

### Verified

- 6モン類×4候補の画像存在、通常1%／20パック25%の個別率、全候補の到達可能性、特別絵のモンスター限定、奪取時の通常絵化を自動検証。

## 1.15.12 — 2026-08-28

### Changed

- レジェンド決勝のChampion AIを3層探索へ強化。自分の現ターン行動列に加え、相手の公開盤面から最大3行動の返しを比較し、その後の自分の既知カード／盤面による最大2行動まで評価する。
- Champion AIの思考予算を85msから140msへ拡張。現在ターン探索、相手応手、次ターン継続へ時間を分割し、公開盤面が同一の枝を除去してスマホ上の上限を維持する。
- 深い探索が時間切れになっても、即時評価の最善手から大きく劣る行動を選ばない戦術下限を追加。
- AI検証CLIへ`--time-a`／`--time-b`を追加し、Legend 55ms対Champion 140msのように実運用と同じ個別予算で比較可能にした。

### Fair play

- 相手の手札内容、山札順、次のドロー、未確定乱数は探索へ使用しない。相手応手は場のモンスターが公開している技だけを候補にし、自分の次ターンも現在すでに判明しているカードだけを使用する。

### Verified

- 合法手・元BattleEngine非変更・相手手札非依存に加え、自分の次ドロー非依存、公開盤面の複数攻撃列、1ms時間切れ時の攻撃取りこぼし防止を自動検証。
- 同一40枚・固定seedの短時間回帰12戦でChampion 7勝／Legend 5勝。140ms設定の初期盤面12判断は平均105.0ms、最大129.9ms。

## 1.15.11 — 2026-08-28

### Added

- 6種の各ブースターへ「収録カード・提供割合」を追加。次の開封回数を参照し、カード名・種類・抽選枠・次の5枚に1枚以上含まれる確率を一覧表示。
- Rare以上、Foil、特別イラストの外観割合と、初回／5回／10回／20回保証を提供内容モーダルへ表示。カード名から通常の詳細画面も開ける。
- 排出表示と実抽選がずれないよう、モン類枠・共通枠・Rare枠の重み定数と候補プールをパック生成器で共有。

### Changed

- ブースター下部の「このゲームに課金要素はありません。」を削除し、ダイヤ入手経路とパック保証の案内は維持。
- 大会エントリーの「使用する40枚」を「使用するデッキ」へ変更。リーダー枠はイラストだけにし、カード名と四隅のLIFE／TP／ATK／DEFを非表示化。

### Verified

- 全6モン類、通常・初回・5回・10回・20回について、各5抽選枠の割合合計、奪取限定カード除外、保証と外観割合を自動検証。ブースター提供画面と横画面レイアウトをブラウザ確認。

## 1.15.9 — 2026-08-27

### Added

- 保存デッキ・カード図鑑などのモンスター詳細へ全9技を表示し、各技が「初期習得」「攻撃修行」「防御修行」のどれに属するかを明記。
- デッキ編集の使用中40枚、デッキ専用予備、未所属候補へ長押し詳細を追加。通常タップの選択・交換操作は維持し、長押し直後の誤交換を抑止。

### Changed

- バトル中のモンスター詳細は、習得済み全技ではなく現在装備している実戦技だけを表示。修行で置き換えた旧技は攻撃選択へ出さない。
- 修行の習得元表示は独自判定ではなく、BattleEngineと同じ`shugyoPools`を参照するよう統一。

### Verified

- 全18体について初期3技・攻撃修行3技・防御修行3技の表示分類を自動検証。技置換後の旧技非表示、長押し後の通常クリック抑止を含む146件のテストと、保存デッキ／バトルの実画面を確認。

## 1.15.8 — 2026-08-27

### Added

- 保存デッキ一覧の各カードへ鉛筆アイコンを追加し、一覧上のモーダルからデッキ名を変更可能にした。大会参加中デッキではUI・アプリ層の両方で変更を禁止。
- 保存デッキ上部の「カード図鑑」の右隣へ「未所属カード」を追加し、ブースター資産へ直接移動して保存デッキへ戻れる導線を追加。
- 青白い多面カットと透過背景を持つ高品質ダイヤ画像を生成し、384px・品質90・alpha品質100のWebPとして追加。

### Changed

- 永久発見履歴の「カード一覧」を「カード図鑑」へ改称し、実所有資産である「未所属カード」と役割を明確化。
- デッキ詳細からデッキ名入力欄と保存ボタンを削除し、リーダー変更・デッキ編集・削除へ整理。
- プレイヤー向けの「総プレイTP」「総TP」表記を「デッキ総TP」へ統一。
- ホーム、ブースター残高、パック価格のCSSひし形／`◆`を共通ダイヤ画像へ置換。

### Verified

- 保存デッキ、名称変更モーダル、詳細画面、カード図鑑、未所属カード、ブースター残高をブラウザで通し確認。新規UIテストを含む全自動チェックを実施。

## 1.15.7 — 2026-08-27

### Added

- レジェンド決勝開始直前の40枚とカードインスタンス別大会内成長を、王座獲得候補スナップショットとして固定。
- 新王者の`championGrowthSnapshot`へLIFE／ATK／DEF成長、習得技、実戦4技を`championDeckSnapshot`と同一transactionで保存。
- 「戴冠時の決勝開始状態を再現する」仕様説明をホームの「遊び方」へ追加。

### Changed

- 次回以降のレジェンド決勝では保存済み王座成長をそのままBattleEngineへ投入し、現チャンピオンへ別大会由来の仮想CPU育成を追加しない。
- 王座防衛40枚は決勝勝利後の奪取結果ではなく、実際に王座戦へ入った決勝開始時40枚へ統一。ユーザー自身の保存40枚には従来どおり決勝後奪取を反映。
- 旧王座データは成長量0として読み込み、次の戴冠からスナップショットv2へ移行。

### Verified

- 決勝開始時スナップショットの固定、チェックポイント復元、決勝中成長の除外、Local/Firebase transaction保存、次の王座戦BattleEngineでの能力再現を自動テストで確認。

## 1.15.6 — 2026-08-27

### Fixed

- ロック導入前から残る大会を完了した際、古い大会40枚が保存デッキを上書きして途中編集カードを消失させる経路を修正。
- 大会終了時は大会40枚を保存しつつ、途中編集で入った差分カードをデッキ専用プールへ退避。復帰カードとプールの同一資産参照は自動除去。
- デッキ保存時にも40枚とプールの重複資産参照を自動修復し、重複エラーで編集不能になる状態を解消。
- 既存の重複データを検出したデッキへ復元枠を付与し、所有履歴から消失カードを選んで通常外観で専用プールへ戻せる復元操作を追加。

### Verified

- ヘンガーをクロノギアへ入替後に旧大会を優勝する再現条件で、ヘンガーが40枚だけへ復帰し、クロノギアが専用プールへ退避され、保存時重複が発生しないことを確認。

## 1.15.5 — 2026-08-27

### Fixed

- 編集ロック導入前の大会中デッキ編集により、使用中40枚とデッキ専用プールへ同一カード資産IDが残った保存データを読み込み時に自動修復。
- 使用中40枚を正としてプール側の重複参照だけを除去し、カードの再採番による架空の資産複製を防止。
- 修復済みデッキをRepositoryへ自動保存し、次回起動後もデッキ編集と保存が可能な状態を維持。

### Verified

- 40枚と予備の衝突、予備内衝突、修復後の再読込、修復後の40枚保存を自動テストで確認。

## 1.15.4 — 2026-08-27

### Added

- 再開可能な大会データの`playerDeck.deckId`を共通判定する大会デッキロックを追加。
- 保存デッキ一覧で対象デッキへ「大会参加中・編集不可」を表示し、詳細画面ではカード閲覧を維持したまま編集操作をLOCK案内へ置換。

### Safety

- 大会参加中デッキでは、名前変更、リーダー変更、40枚／専用プール編集、削除を禁止。他の最大4デッキと新規作成は従来どおり利用可能。
- UI入口だけでなく、デッキ編集画面の直呼び、40枚保存時、削除確認前・確定時にもロックを再判定し、進行中大会と保存40枚の不整合を防止。
- 大会終了・cleared・破損したチェックポイントではロックを残さない。

### Verified

- 1280×720横画面で大会対象デッキの一覧バッジ、LOCK案内、カード閲覧を確認。名前保存、リーダー変更、デッキ編集、削除ボタンが各0件であることと、ブラウザ警告・エラー0件を確認。

## 1.15.3 — 2026-08-27

### Fixed

- 特殊合体カードの名前横に表示していた冗長な「合」マークを削除。
- 特殊合体の一覧カードと手札カードでは、イラストを名前帯の下から開始するよう21px補正。名前帯を省略する盤面カードと詳細画像には補正を適用しない。
- `fusion-028`ユーマだけ背景横位置を50%から8%へ変更し、左側にある顔が切れないようイラストを右方向へ移動。

### Verified

- 1280×720横画面で特殊合体36体の名前帯、画像上端、最下段までを目視確認。ユーマの顔全体がカード内へ収まり、「合」表示が0件、ブラウザ警告・エラーが0件であることを確認。

## 1.15.2 — 2026-08-27

### Fixed

- 基本モンスター18体、Training 3枚、修行2枚、既存ブリーダー20枚だけ、カード名の帯が終わる位置からイラストを描画するよう変更。顔や頭部を帯で隠さず、クロノギア以降の単体画像には位置補正を適用しない。
- 特殊合体アトラスの段高が`234 / 235 / 230 / 205 / 178 / 172px`と不均一だったため、固定6等分の座標参照を廃止。実境界から36体を個別WebPへ切り出し、上段から最下段まで隣接カードが混入しない描画へ変更。
- ブルードリルも同じ個別画像経路へ統一し、特殊合体カードの一覧・詳細・バトル表示で同一の画像を使用。

### Verified

- 旧カードのイラスト上端が名前帯の下21px、クロノギアと新ブリーダーが従来どおり0pxであることを1280×720横画面で計測。
- 管理者ツールの特殊合体36体を先頭・中段・最下段まで確認し、全カードが`special-fusions/fusion-001.webp`〜`036.webp`を参照し、別カードの帯状混入がないことを確認。

## 1.15.1 — 2026-08-27

### Fixed

- 基本モンスター18体、Training、修行、既存ブリーダー20枚で、同じアトラス画像を背景と中央画像へ二重描画していた構造を廃止。カード全面へ1回だけ描画するよう統一。
- 特殊合体36体の上端に下層の同一／隣接イラストが帯状に露出する問題を修正。ブルードリルを含む全カードで疑似画像レイヤーを無効化。
- Foilの光が1周期内で二度走るように見える繰り返し背景を、非反復の単一スイープへ変更。

### Rules and migration

- Foilと特別イラストをモンスターカード限定へ統一。10パック保証のFoil抽選も必ずモンスターから選ぶ。
- 既存の未所属資産、未確認パック、保存40枚、デッキ専用プールに育成／ブリーダーのFoil・特別絵があった場合、枚数・性能を変えず通常外観へ自動正規化。

### Verified

- 1280×720横画面の管理者ツールで基本モンスター、ブリーダー40枚、特殊合体36体を目視し、全対象の疑似画像が`content:none`・`display:none`であることを確認。
- Foil保証パックでFoilがモンスター1枚だけに付与され、育成／ブリーダーFoilが0枚であることを確認。光の位置が170%から-70%へ一度だけ移動し、画面外で周期更新することを確認。


## 1.15.0 — 2026-08-27

### Added

- URLの`?admin=1`でのみホームへ表示される読取専用「管理者ツール」を追加。
- 基本カード69枚、特殊合体36種、ブースター特別イラスト6種の計111件を、種類・入手経路・検索語で確認できる全カードカタログを追加。
- 6モン類のパックと、通常抽選／新モンスター保証／Foil保証／特別絵保証を選び、任意seedで本番同等の封印破壊・配札・反転・レアリティ演出を再生できるガチャプレビューを追加。

### Safety

- 管理者プレビューはRepository、ダイヤ、未所属資産、保存デッキ、開封回数へ一切書き込まない純粋な表示専用経路として分離。
- プレビュー画面へ「ADMIN PREVIEW / 資産変更なし」を常時表示し、完了後は管理者画面へ戻る。

### Verified

- 1280×568横画面で111件表示、特別絵6件フィルター、クロノギア検索、6パック選択、特別絵保証SHOWCASEを実操作確認。
- プレビュー前後でダイヤ600・初回無料が不変であることと、ブラウザ警告・エラー0件を確認。
- 120件の自動テストとv1.15.0 GitHub Pages/PWAビルドが成功。

## 1.14.1 — 2026-08-26

### Fixed

- 未所属カード一覧を固定幅グリッドへ変更し、モンスター／Training／修行／ブリーダーのカード本体を同一寸法に統一。
- ブースター限定モンスターの単体イラストへ旧アトラス用の中央画像を重ねていた二重描画を廃止。上部の継ぎ目と不自然な重複を解消。

### Improved

- パック開封を、回転魔法陣と粒子を伴う封印破壊、5枚の段階配札、個別3Dフリップ、Rare／Foil／Showcase専用バーストの多段演出へ刷新。
- 「すべてめくる」も瞬間表示せず、左から順にカードを開くよう変更。`prefers-reduced-motion`では短縮表示を維持。

### Verified

- 1280×568横画面でカード4種の外形がすべて142×197px、単体モンスター画像の疑似要素が`display:none`であることをブラウザ計測。
- 封印破壊、配札、個別フリップ、一括開封、資産受取、未所属一覧を実操作確認。

## 1.14.0 — 2026-08-26

### Added

- 完全無料の常設6モン類ブースターを追加。1パック5枚・300ダイヤ、新規600ダイヤ＋初回無料、モンスター1枚以上・Rare以上1枚以上を保証。
- ブースター限定モンスター6体（クロノギア、アルケミア、ミストレイ、ノクティス、ヴォルファング、グラトン）と54技、共通BattleEngineで動く6特性を追加。
- 通常版6枚と特別版6枚の文字なし縦長イラストを組み込みImageGenで制作し、高品質WebPとして追加。
- 封印破壊、粒子、5枚ファン、個別めくり、Rare/Foil/Showcase表示を備えたパック開封演出を追加。
- 未所属カード資産、保存デッキ専用プール、40枚との1対1交換画面を追加。
- 大会勝利・優勝のダイヤ報酬を追加。operation IDで再開時の二重受取を防止。

### Rules

- `breeder-029`〜`040`を奪取限定とし、ブースターから除外。通常CPUは引き続き使用するため、勝利後の奪取経路を維持。
- `monster-019`〜`024`をブースター限定とし、通常CPU生成から除外。他プレイヤーのLegend公開40枚には登場可能。
- 特別イラスト／Foilは奪取不可。奪取候補では同じ性能の通常イラスト・通常仕上げへ正規化。
- 初回およびモン類別5パックごとに新モンスター、10パックごとにFoil、20パックごとに特別イラストを保証。

### Persistence and delivery

- パック結果を演出前にLocalStorage／Firestore transactionへ保存し、タスクキル後の未確認パックを再開可能にした。
- FirebaseをUIから分離したRepositoryへガチャ経済・デッキ＋資産同時保存APIを追加。
- 新規カード絵はPWA初回プリキャッシュから外し、初回閲覧時にランタイムキャッシュするよう変更。

### Verified

- 114件の自動テストとv1.14.0 GitHub Pages/PWAビルドが成功。
- iPhone横画面相当1170×540で、ホーム、6パック一覧、封印、5枚めくり、資産受取、未所属カード、デッキ専用プール交換をブラウザ操作確認。
- 通常版・特別版12ファイルのWebP配信と、PWA初回プリキャッシュ除外を確認。

## 1.13.0 — 2026-08-26

### Added

- 保存デッキの「新規作成」へ、無機・創造・幻霊・魔族・獣族・怪物の6種から固定40枚を選べるスターター選択画面を追加。
- 後攻5ターン目の開始時に一度だけ発生する`fusion-unlocked`イベントと、金属紋章・回転陣・2枚のカードが収束する合体解禁専用演出を追加。

### Changed

- 初期のモン類ブリーダー12枚を、硬化装甲・装甲解析・省力設計・機能停止・連続顕現・幽体回避・魔力注入・群魔の共鳴・野生の活力・生命の息吹・威嚇咆哮・群体進化へ改称。
- 改称対象の効果判定を表示名ではなく既存の`breeder-009`〜`breeder-020` IDへ固定し、既存セーブ互換と将来の再改名耐性を維持。

### Verified

- 6スターターすべてが40枚、同名上限内、各モン類9体＋専用ブリーダー4枚の合法デッキであることを自動検証。
- 後攻5ターン目だけ解禁イベントが1回発生し、先攻5ターン目や後攻6ターン目には再発しないことを自動検証。
- 105件の自動テストとv1.13.0 GitHub Pages/PWAビルドが成功。

## 1.12.0 — 2026-08-26

### Changed

- バトル背景、基本モンスター18体アトラス、特殊合体36体アトラス、ブルードリル、Training・修行・既存ブリーダーアトラス、追加ブリーダー20枚を高品質WebPへ移行。
- LIFE・コスト・ATK・DEFの透過バッジ4枚を可逆WebPへ移行し、透過境界と表示品質を維持。
- PWAキャッシュをv1.12.0へ更新。旧PNG/JPEGは配信対象から除去し、CSS・カードレンダラー・テストをWebP参照へ統一。
- PWA導入互換性を守るため、合計約59KBのManifest/iOS用PNGアイコンと編集元SVGだけは例外として維持。

### Optimized

- ゲーム内ランタイム画像29ファイルを12,708,373 bytesから6,831,672 bytesへ削減（約46%減）。
- GitHub Pages/PWA配信物をv1.11.0の13,262,264 bytesから7,386,206 bytesへ削減（約44%減、ファイル数は96のまま）。
- イラストはWebP quality 90～92、effort 6、alpha quality 100で変換。元画像とのPSNRは最小約38.25dB、個別ブリーダーカードは約40.99～42.84dBを維持。

### Verified

- 元画像と変換後のモンスターアトラス・個別カードを原寸目視比較し、輪郭、暗部、発光、主要ディテールに実用上の差がないことを確認。
- 29ファイルすべてのRIFF/WEBPシグネチャ、CSSの旧PNG/JPEG参照ゼロ、追加ブリーダー20枚の容量上限を自動テストで確認。
- 100件の自動テストとv1.12.0のクリーンGitHub Pages/PWAビルドが成功。

## 1.11.0 — 2026-08-26

### Added

- 汎用8枚と各モン類2枚ずつの専用12枚、合計20枚の新ブリーダーカードを追加。既存20枚は維持し、マスター総数を40枚へ拡張。
- 戦線整理、素材探索、融合強化、状態回復、反動攻撃、全体防御、TP前借り、逆境強化、オーバークロック、自動修復、ATK/DEF入替、撃破耐性、残響追撃、手札帰還、血の契約、呪印、狩猟報酬、群れ防御、暴食、捕食進化を共通BattleEngineへ実装。
- 複数の手札・探索・捕食候補を、人間プレイヤーがスワイプ後に選べるブリーダー専用選択モーダルを追加。
- 新規20枚それぞれに組み込みImageGenで文字なし正方形イラストを生成。768×768・JPEG品質90へ最適化した個別アートと生成プロンプト記録を追加。

### Changed

- 大会別CPUデッキ生成器へ新規汎用・モン類ブリーダーを追加し、従来どおり1デッキ4枠の範囲で選別するよう更新。
- PWAの大量事前キャッシュを考慮し、生成画像20枚を約50MBのPNGから合計約3.5MBの配信用JPEGへ最適化。

### Verified

- 新規20効果の合法手・数値・持続・撃破時処理・手札移動・合体プレビューを自動テストで確認。
- 20枚の一覧と実カード表示を目視し、文字混入、主題切れ、画像欠損がないことを確認。
- 99件の既存＋追加テストが成功。追加画像20枚を含むv1.11.0のGitHub Pages/PWA配信物（96ファイル、13,262,264 bytes）を正常に生成。

## 1.10.1 — 2026-08-25

### Changed

- モンスターのLIFE・ATK・DEFバッジを同じ大きさへ統一し、従来のATKよりわずかに拡大。コスト円の確定済みサイズは変更していない。
- カード上のアタッカー・バランス・タンクの役割アイコンを撤廃。役割名はカード詳細でのみ確認できる。
- 手札・保存一覧などのモンスター名をカード最上部へ移動し、表示面積の小さい盤面カードでは名前帯を非表示にした。アクセシブル名、詳細画面、バトルログの名前は維持している。

### Verified

- 844×390の実バトルでLIFE・ATK・DEFが各22px、コストが19px、役割アイコン0件、手札の名前帯が上端から1px、盤面の名前帯が非表示になることを確認。
- 1280×720でLIFE・ATK・DEFが各37px、コストが31pxになることを確認。ブラウザconsole errorは0件。
- 92件の自動テストとv1.10.1 Pages/PWAビルドを確認。

## 1.10.0 — 2026-08-25

### Added

- プロジェクト提供のLIFEハート、コスト円、ATK剣、DEF盾を、元の金縁・色・質感を保った透過512px PNGとしてカード描画へ追加。

### Changed

- バトル手札の「YOUR HAND / 操作案内 / 山札・墓地」見出し行を撤廃。山札・墓地枚数は左HUDに残し、空いた高さをカード表示へ割り当てた。
- 横画面の手札領域を33dvh・最大224pxへ拡大し、手札カードを高さ100%で表示。カード間隔と外周余白も広げ、外付けバッジを欠けずに表示する。
- LIFE/DEFを大きな基準バッジ、コスト/ATKを約25〜30%小さい補助バッジとし、四隅を従来よりカード外側へ移動。
- 保存40枚、カード一覧、カード奪取、リーダー選択でも外付けバッジ同士が重ならないよう、カード間隔とスクロール外周余白を拡大。

### Asset provenance

- 4点は2026-08-25にプロジェクト所有者から提供されたJPEGを正本とし、生成し直していない。背景抽出用ImageGenはLIFEの市松模様を画像化したため不採用とし、均一な無彩色背景だけを除去して透過PNGへ正規化した。

### Verified

- 844×390の実バトルで手札見出しが0件、手札高さ128px、カード高さ101pxとなり、5枚のLIFE/コスト/ATK/DEFが欠けずに表示されることを確認。
- 1280×720の実バトルで手札高さ224px、カード132×183px、LIFE/DEF 48px、コスト31px、ATK34pxを確認。
- 1280×720の保存40枚でカード本体間22pxを確保し、10列の外付けバッジが隣接カードと重ならないことを確認。ブラウザconsole errorは0件。
- 91件の自動テストとv1.10.0 Pages/PWAビルドを確認。

## 1.9.1 — 2026-08-25

### Changed

- 全カードの背景とは別に、アトラス内の1体分を正しい縦横比で名前帯より下へ重ねる頭部セーフエリアを追加。モンスターは縦長、育成・ブリーダー・特殊合体は正方形の全体像を維持する。
- カード詳細の固定横長画像を廃止し、モンスターは3:4、育成・ブリーダー・特殊合体は1:1で表示。
- 右上コスト円を通常カード・高さ430px以下の手札とも縮小。
- 保存デッキ、カード一覧、カード奪取、リーダー選択、バトル手札、盤面のカード間隔と外周余白を拡大。
- 大会内育成を示す「大会+数値」バッジを撤廃。成長後のLIFE/ATK/DEF数値への反映は維持。

### Verified

- 1280×720の保存40枚で10列すべての頭部・四隅アイコン・カード間隔を目視確認。
- 844×390の手札でコスト円と隣接カードが重ならず、育成バッジが生成されないことを確認。
- 通常モンスターとブリーダーの詳細を開き、画像が横長に切れず全体表示されることを確認。
- 89件の自動テストとv1.9.1 Pages/PWAビルドを確認。

## 1.9.0 — 2026-08-25

### Changed

- バトルカードを全面イラストへ変更し、LIFEを左上のハート、召喚/使用コストを右上の青い円、ATKを左下の剣、DEFを右下の盾としてカード外周へ重ねるレイアウトへ刷新。
- カード面の特性・効果文を外し、タップ後の詳細画面へ集約。名前と役割マーク、動的な四隅数値、状態変化だけをバトル中に表示する。
- バトルHUD、ターン、ログ、手札案内、操作ボタンの文字を通常画面と高さ430px以下の両方で拡大。
- 相手盤面のカードを正立表示へ戻し、プレイヤー側から四隅数値と名称をそのまま読める向きへ統一。
- 特殊合体/育成カードの正方形アトラスを縦長カードへ強制変形せず、中央の1体画像は縦横比を維持し、外周だけ同画像で補完する表示へ変更。

### Verified

- 特殊合体36体を全面イラストの実カードとして前半・後半に分けて目視し、全IDが正しい個体へ対応し、ブルードリルを含め中央画像の欠け・セルずれがないことを確認。
- 844×390のバトルQAで四隅バッジが手札から欠けず、拡大したHUD/ログ/操作文字が3列レイアウト内へ収まることを確認。
- 88件の自動テストとv1.9.0 Pages/PWAビルドを確認。

## 1.8.0 — 2026-08-25

### Added

- 大会表、進行中バトル、カード奪取の未確定選択を端末とCloud Firestoreへチェックポイント保存し、再読込・PWA終了・タスクキル後にホームの「大会の続きから」から復元する機能。
- BattleEngine、TournamentRun、CardStealSessionのseed/RNG状態を含む復元APIと、古い非同期保存による巻き戻りを防ぐ単調更新時刻・終了tombstone。
- 相手ターンや演出中にタップした手札1枚を予約し、自分の入力受付開始時に自動選択する操作。

### Changed

- 16人トーナメントのCPU同士の結果は内部ではseed付きで先に確定しつつ、表示上はプレイヤーの同ラウンド終了まで「対戦中」として伏せる。
- 縦向き案内をページ直下の単一オーバーレイへ統一し、バトル画面側の重複表示を削除。

### Verified

- 87件の自動テストとv1.8.0 Pages/PWAビルドを確認。
- 大会開始直後、試合途中の双方でページを再読込し、同じ大会表・山札/手札・盤面・LIFE/TP・バトルログへ復帰することを確認。
- 390×844で縦向き案内が1枚だけ中央に固定表示され、844×390/667×375の横画面を妨げないことを確認。

## 1.7.0 — 2026-08-25

### Added

- 通常合体でメインカードと素材カードが左右から収束し、完成カードとSP増加を見せる全画面演出。
- 特殊合体専用の紫光、二重紋章、放射光、粒子、特殊個体イラストと名称を使った上位演出。
- 標準/高速の演出時間差と、`prefers-reduced-motion`利用者向けの短縮表示。

### Changed

- Training・修行カードの効果文から「大会中継続」を外し、同じ内容を遊び方と7ステップチュートリアルへ集約。
- ホーム最下部の保存方式、debug seed、Sim番号を通常ユーザーから非表示化。localhostの`?debug=1`だけで確認可能とし、同期失敗時は技術詳細ではなく安全な端末保存中であることだけを表示。
- 合体演出内では盤面用の「行動済」帯を出さず、カードイラストを広く表示。実際の行動権は正式ルールどおり回復しない。

### Verified

- 78件の自動テストとv1.7.0 Pages/PWAビルドを確認。
- 844×390の特殊合体と667×375の通常合体で、素材収束、完成カード、合体名、SP増加表示が画面内に収まることを確認。
- 通常ホームのDOMにdebug seed、Sim番号、保存方式フッターが存在しないことを確認。

## 1.6.1 — 2026-08-25

### Added

- CPU同士の裏試合勝者へ、実デッキ内のTraining・修行による大会内育成をseed付きで蓄積する進行。
- 2回戦・準決勝・決勝の対戦相手へ、それぞれ1・2・3戦分の育成状態を共通BattleEngine経由で反映。
- 対戦前画面に、相手の「勝ち上がり育成」戦数、能力上昇合計、新技数を表示。

### Balance

- 裏試合育成は大会別の軽量TP予算を使い、Bronze 5 / Silver 6 / Gold 7 / Legend 8。修行は1試合最大1回とし、カードを持たないCPUへ架空の強化を与えない。
- CPUブラケット勝敗評価へ、それまでの能力上昇と新技習得を小さく加点。

### Verified

- 後半戦の相手が育成状態を保持してBattleEngineへ入ること、同一seedで完全再現できることを自動テスト。
- 40seed平均の決勝時総能力上昇はBronze +33.4、Silver +44.6、Gold +57.0、Legend +61.2。複数モンスターへ分散し、最も育った1枚の80seed平均は+13.6～20.9。
- Legend大会を200回生成・3ラウンド進行した平均処理時間は16.69ms。完全なCPU戦シミュレーションを11試合追加せず、スマホ負荷を抑えた。
- 75件の自動テスト、v1.6.1 Pages/PWAビルド、844×390と667×375の対戦前表示を確認。外側overflowとコンソールwarning/errorは0件。

## 1.6.0 — 2026-08-25

### Added

- 保存デッキ内の任意のモンスターをリーダーへ変更し、一覧・大会・公開Legendデッキ・王座へ反映する機能。
- 一度でも所有した基本カードと、プレイヤーが一度でも成功させた特殊合体を永続表示する「カード一覧」。種類別フィルター、収集数、カード詳細、特殊合体レシピ詳細を収録。
- LocalStorage / Cloud Firestore間で所有・発見履歴を安全に和集合するRepository APIと、既存保存デッキからの自動移行。

### Fixed

- Training 3種と修行2種に動的な効果説明を復元し、マスター再生成後も欠落しない検証を追加。
- 選択したリーダーがカード交換後もデッキ内に残る場合、先頭モンスターへ意図せず戻る問題を修正。

### Verified

- リーダーの保存・不正選択拒否・放出時フォールバック、カード履歴の追加専用統合、敗戦時を含む特殊合体発見、育成カード説明を自動テスト。
- 自動テスト72件とGitHub Pages/PWAビルドを確認。
- 844×390と667×375で外側overflowなしのデッキ詳細・リーダー候補・カード一覧・特殊合体詳細を操作し、コンソールwarning/error 0件を確認。

## 1.5.1 — 2026-08-25

### Changed

- Firebaseプロジェクト`monster-collections`のWebアプリ設定を本番クライアントへ接続。
- GitHub Pages/PWAのキャッシュバージョンを1.5.1へ更新し、Firebase接続設定を既存インストールにも確実に配信。

### Security

- Firestoreへのアクセスは匿名Authenticationとリポジトリ管理の`firestore.rules`を前提とし、Firebase障害時は既存のLocalStorageデータを保持。

### Verified

- `monster-collections`の`(default)`データベースへFirestore Rules/Indexesをデプロイし、Rulesのコンパイルとrelease成功を確認。

## 1.5.0 — 2026-08-25

### Added

- ホームの「遊び方」から開始できる7ステップの横画面チュートリアル。勝利条件、スワイプ召喚、技攻撃、Training/修行、合体、TP、カード奪取を順に説明。
- Cloud Firestoreの `legendDecks` 公開スナップショット。Legend資格を得た保存40枚を所有者だけが更新し、他プレイヤーのレジェンド杯通常枠へ最大14デッキまで投入。
- 公開40枚を所有者の非公開保存デッキ・プロフィールと照合するFirestore Security Rulesと、破損/旧マスター文書を大会投入前に除外する合法性検証。
- 特殊合体36体を実ゲームの横長カード比率で一括確認する `tools/fusion-art-qa.html`。

### Changed

- 合体SPは従来の平均×1.20式を維持しつつ、最低でも「メイン現在SP＋素材現在SPの10%（切上げ）」になる保証を追加。育成済みメインの弱体化と手札モンスター滞留を防止。
- 手札モンスターへ同一大会で持ち越したTraining/修行の現在LIFE/ATK/DEFを表示し、`大会 +N` バッジを追加。成長寿命は大会終了までのまま維持。
- Legend杯は他プレイヤーの合法な公開デッキを優先して16人表へ入れ、不足枠だけを従来の制約付きLegend CPUで補完。決勝は引き続き現チャンピオン固定。
- PWAキャッシュと静的アセット版を1.5.0へ更新。

### Fixed

- ブルードリルの角・口先・頭部がアトラスセル境界で切れる問題を、専用の安全余白付きイラストへ差し替えて修正。
- 「遊び方」下の `距離廃止版ルール` と旧距離廃止注記を通常UIから削除。
- Champion AIの非公開手札不変テストを、端末負荷で探索深度が変わらない決定的探索モードへ分離。本番のスマホ向け思考時間上限は維持。

### Verified

- 67件の自動テストとGitHub Pagesビルドを確認。
- 844×390で7ステップすべてが680×341px内、内部スクロールなし、横縦overflowなしで表示され、大会選択まで遷移することを実ブラウザーで確認。
- 特殊合体36体を166×114pxの実カード画像比率で確認。ブルードリルは専用画像、他35体は正しいアトラスセルを使用し、欠落画像と追加の致命的な部位切れがないことを確認。
- Gold同士6戦の小標本で合体発生試合83.3%、特殊合体発生試合83.3%、40T判定0%、平均9.83ラウンドを確認。長期バランスの確定値ではない。
- Firestore Repository mockでLegend公開、他ユーザー読込、自分の除外、削除連動、16人枠への混在、決勝Champion固定を確認。

## 1.4.1 — 2026-08-24

### Fixed

- バトル画面の再描画後もログを最下段へ自動追従させ、常に最新イベントが見えるように修正。
- CPUの最終演出中に先行入力された手札カードのタップを予約し、演出完了後に1回のタップで選択状態へ反映。
- 予約されたタップへ即時の枠色・明度フィードバックを追加し、入力を受け付けたことを明示。

### Added

- ログoverflowとCPU最終演出中のタップを決定的に再現する844×390開発用QA画面。

### Verified

- 60件の自動テストとGitHub Pagesビルドを確認。
- 844×390で、ログ最下端への追従、1回のタップによる予約表示、演出終了後の同一カード選択を実ブラウザーで確認。

## 1.4.0 — 2026-08-24

### Added

- 修行で5個目以降の技を習得した直後に、現在の実戦4技のどれかと入れ替えるか、習得だけにするかを選ぶ必須フロー。
- 5択を `BattleEngine` の正式な合法行動にし、人間・全CPU・シミュレータで同じ4枠制限を使用。
- 特殊合体36形態をレシピ順に収録した6×6アトラスと、Training 3種・修行2種・ブリーダー20種を収録した5×5アトラス。
- 844×390 / 667×375のUI確認と5技目入替を再現する開発用viewport harness。

### Changed

- モンスターカードのLIFEを現在値だけの表示へ変更し、モン類の印章アイコンを撤去。
- 標準速度を緩和し、CPU行動間隔600ms、通常イベント850ms、重要イベント1250msへ延長。
- 能力増減を差分付き（例 `⬆︎ ATK +5` / `⬇︎ LIFE -12`）で約1.2秒表示し、矢印提示後に数値を更新。
- タッチ端末でカードhover/選択が位置を動かさないようにし、バトル中のbody/appをviewportへ固定。
- 新規画像を高品質JPEGへ最適化し、2アトラス合計を約7.7MBから約1.4MBへ削減。

### Verified

- 58件の自動テスト、GitHub Pagesビルド、全AI完走を確認。
- 844×390でbody/htmlが844×390、scroll 0、`position: fixed` / `overflow: hidden`であることを実測。
- 5技目の4入替候補＋習得のみを全表示し、「かみつき」→「インフェルノ」の入替とログ反映を実操作。
- 増減表示中に旧値から新値へ切り替わり、切替後も表示が残る順序を実ブラウザーで確認。

## 1.3.0 — 2026-08-24

### Added

- 選択済み手札カードを盤面へスワイプして、召喚、Training、修行、ブリーダー、合体を実行する直接操作。
- 盤上モンスター詳細の実戦技タップから、合法な攻撃対象を盤面上で選ぶ攻撃フロー。
- 修行前の習得候補一覧と確認/キャンセル、seed付きランダム技習得、小差のRank重み。
- Training/修行の発光演出、攻撃カードの突進、能力増減前の `⬆︎` / `⬇︎` 表示。

### Changed

- カード右上TPを `②` 形式へ変更し、モンスター名左へアタッカー/バランス/タンクの小型マークを表示。
- 手札モンスターから特性文と「モン類 / 役割」文字列を除き、詳細は現在覚えている技だけに簡略化。
- 右側の「行動を選ぶ」一覧を撤去し、ログ、速度、選択解除、ターン終了だけに整理。
- 相手盤上カードを180度反転し、ホームの代表モンスターをイラスト領域だけの表示へ変更。

### Verified

- 844×390と667×375で、スワイプ召喚、対象Training、修行候補/抽選、技選択/対象攻撃を実操作。
- 667×375でbodyの縦横overflowなし、技詳細は2列のまま主要情報と覚えている技を同時表示。

## 1.2.1 — 2026-08-24

### Changed

- バトル画面をUIリファレンス準拠の「左ステータス / 中央テーブル / 右コマンド」構成へ全面再設計。
- 相手の伏せ手札、LIFE/TPゲージ、山札/墓地、上下3枠盤面、常時表示ログを一画面へ整理。
- 手札カードを高さ基準で縮尺し、モンスター画像・TP・LIFE/ATK/DEF・効果が潰れにくい表示へ変更。
- Training、修行、ブリーダーのカード種別を色と印章で識別可能に変更。
- 高さ430px以下では大会の対戦案内を右側へ移し、16人ブラケットの縦領域を確保。

### Fixed

- 844×390前後の横画面で、行動候補が手札へ重なりカード下部が欠ける問題。
- 低い横画面で大会ブラケット下部が切れ、全8試合を確認できない問題。
- デバッグseed表示を本番UIへ出さず、デバッグモード内だけに限定。
- PWA更新時に新Service Workerへ切り替わったら一度だけ自動再読込し、旧UIキャッシュが残る問題。

## 1.1.0 — 2026-08-24

### Added

- GitHub Pagesのサブパスに対応したWeb App ManifestとService Worker。
- 192px、512px、1024px maskable、iOS 180pxのアプリアイコン。
- standalone横画面、オフラインアプリシェル、Firebase CDNの通信失敗時cache fallback。
- 対応ブラウザーのホーム画面に「アプリに追加」導線、iOS向け追加手順。
- 各試合開始時の双方40枚シャッフルを同一seed再現・異seed変化まで確認する回帰テスト。
- 同じページ内で再挑戦しても、通常プレイの大会seedを使い回さないrun seed発行器。

### Verified

- シャッフルは初期3枚を引く前に実行され、山札切れ時も墓地を再シャッフルする。
- PWAのstart URLとscopeは相対指定で、`/monster_collection/`配下から外れない。

## 1.0.0 — 2026-08-24

プレイテスト可能な第1完成版。

### Added

- Sim8.7完全展開版の18モンスター、162技、20ブリーダー、36特殊合体をJSONマスター化。
- 人間UIと全CPUが共有するseed注入可能な `BattleEngine`。
- 正式ドロー、召喚酔い、行動権、合体解禁、総プレイTP先攻、40ラウンド残LIFE判定。
- Training、修行、トーナメント内成長、最大9習得技・実戦4技。
- Bronze / Silver / Gold / Legend / Championの非チートAIと時間制限付き探索。
- 7テーマ、ランク別候補選別、狙い/実成立/偶発レシピを分離するCPUデッキ生成器。
- 16人ブラケット、プレイヤー4試合、レジェンド決勝だけ現チャンピオンとなる大会進行。
- 5枚提示、最大2枚、同数放出、最終確認、キャンセル可能なカード奪取。
- 最大5保存デッキ、デッキ単位資格、敗退時保存。
- LocalStorage / Firebase / 障害耐性Repositoryと王座version transaction。
- スマホ横画面UI、safe-area、標準/高速演出、縦向き案内。
- GitHub Pagesビルド、サブパスpreview、Actions deploy。
- AI比較、CPUデッキ統計、実エンジン大会プレイテストCLI。
- オリジナルの大会背景と18体モンスター肖像atlas。

### Changed from Sim8.7 by explicit user amendment

- 距離、得意距離、遠/中/近レーン、移動行動を廃止。
- 盤面を距離のない3つの汎用枠とし、実戦技は任意の合法対象へ使用可能に変更。
- 旧距離データは原典追跡用 `legacyDistance` としてのみ保持。

### Fixed during playtest

- 未来ラウンドへ誤ってWINが表示されるブラケット表示。
- CPU名の姓が過度に重複する生成。
- 844×390でブラケット下部がはみ出す問題。
- CPU演出待機中に試合が終了した際、終了済みstateへ行動を適用する競合。
- 特殊合体後も基礎特性の状態・表示文が残る問題。
- 反動ダメージが「被攻撃」特性を発動する問題。
- Silverのターン終了だけ絶対評価値、他行動は差分値だったAIスコア不整合。
- 上位AIが下位AIより明白に悪い単発手を選ぶ探索退行。
- 初期40枚に修行とブリーダーがなく、序盤の学習と耐久性が不足していた構成。
- Legend生成時の素材削減で狙った特殊合体レシピが壊れる場合。

## Generated raster asset prompts

生成には組み込みImageGenを使用し、成果物を `assets/images/` へ保存した。カード名、数値、効果、UIは画像へ焼き込んでいない。

### `battle-arena.png`

```text
A wide 16:9 dark fantasy tournament arena background for a Japanese mobile landscape web card battle game. Ancient circular stone colosseum at twilight, deep navy and charcoal palette, subtle teal magical light, warm amber braziers, ornate but restrained, clear central battlefield area, atmospheric depth, polished premium trading-card-game illustration. No people, no monsters, no cards, no logos, no UI, no symbols that resemble text, no letters, no numbers. Keep the center and lower third visually quiet so dynamic HTML game pieces remain readable. Full-bleed raster game background.
```

### `monster-atlas.png`

```text
Use case: stylized-concept
Asset type: 6-column by 3-row monster portrait sprite atlas for a dark fantasy mobile trading-card web game
Primary request: Create one clean 6x3 atlas containing exactly 18 distinct original monster portrait illustrations, one subject per equal cell, in this fixed left-to-right order.
Row 1: (1) towering rune-carved stone monolith guardian, (2) hovering bronze-and-teal mechanical automaton, (3) massive craggy stone golem, (4) elegant blazing phoenix firebird, (5) slender friendly silver cosmic alien, (6) masked one-eyed arcane sage.
Row 2: (7) carnivorous flowering vine plant, (8) round soft white mochi-like creature with ears, (9) regal water elemental woman, (10) winged violet demon fairy, (11) headless black-armored knight carrying a spectral helm, (12) horned crimson western dragon.
Row 3: (13) blue-white lightning wolf, (14) compact athletic golden-furred ape fighter, (15) green feathered raptor dinosaur, (16) eerie masked shadow jester, (17) large armored teal caterpillar-worm, (18) translucent pale-blue sheet-like ghost.
Style/medium: polished original Japanese dark-fantasy trading-card illustration, painterly raster, consistent art direction, readable at thumbnail size, not copied from any existing franchise designs
Composition/framing: exact equal 6 columns and 3 rows, centered bust or three-quarter subject in every cell, consistent portrait scale, clear thin gutters, no subject crossing cell boundaries
Lighting/mood: dramatic rim light, deep navy backgrounds, faction accents in stone/teal/gold/violet/red/blue
Constraints: exactly 18 cells and exactly one creature per cell; no empty cells; no text, letters, numbers, UI, card borders, logos, symbols, captions, watermarks, or baked-in stats; keep all important anatomy away from gutters; 3:2 full atlas aspect ratio
Avoid: existing copyrighted character likenesses, photorealism, chibi-only proportions, messy collage, variable cell sizes
```

### `special-fusion-atlas-v1.jpg`

組み込みImageGenの生成PNGを目視確認後、品質90のJPEGへ変換した。元PNGは生成物保管領域に残し、PWAには軽量版だけを収録する。

```text
Create one production-ready square raster sprite atlas for a dark high-fantasy collectible card battle game.

LAYOUT IS CRITICAL:
- Exactly 6 columns by 6 rows, 36 equal-size rectangular cells in row-major order.
- Each cell is a separate full-bleed monster portrait illustration.
- Perfectly aligned cell boundaries; no gutters, no padding, no frames, no separators.
- No text, letters, numbers, symbols, logos, UI, card frames, watermarks, or labels anywhere.
- Keep each monster centered with head and upper body clearly readable after a cell is cropped.
- Consistent dramatic painterly style: premium Japanese dark-fantasy card art, realistic painted textures, strong silhouette, cinematic rim lighting, deep atmospheric backgrounds, saturated accent color, high contrast.
- Every cell must be visually distinct. Avoid placing important anatomy across cell boundaries.
- Do not make a contact sheet with captions. This must be a seamless crop-ready game asset.

CELL CONTENT IN EXACT ROW-MAJOR ORDER:
Row 1:
1. elegant psychic fairy fused with sleek silver alien machine, cyan-magenta energy wings;
2. nocturnal butterfly witch fused with giant armored worm, violet moonlight;
3. undead knight fused with immortal firebird, blazing armored avian warrior;
4. frost-armored headless knight fused with blue lightning wolf, icy electric warrior;
5. muscular martial-arts hamster fused with lightning wolf, compact blue-white beast fighter;
6. blue lightning wolf fused with carnivorous healing plant, botanical electric beast.
Row 2:
7. muscular hamster fused with radiant fairy, agile winged prizefighter;
8. dark obsidian hamster fused with black monolith, blocky cursed martial beast;
9. dinosaur fused with stone golem, ankylosaur-like rock fortress;
10. dinosaur fused with golden sun-mask deity, radiant armored reptile;
11. ghost fused with fallen samurai armor, spectral ronin;
12. ghost fused with water spirit maiden, eerie drowned apparition.
Row 3:
13. giant worm fused with dinosaur, plated venomous reptilian insect;
14. giant worm fused with lightning wolf, blue horned burrowing predator;
15. demonic jester fused with red dragon, apocalyptic flame demon;
16. demonic jester fused with pink rice-cake creature, beautiful sakura death spirit;
17. black monolith fused with muscular hamster, wild stone brawler;
18. black monolith fused with dinosaur, colossal prehistoric fortress wall.
Row 4:
19. stone golem fused with demonic jester, eldritch apocalypse idol;
20. stone golem fused with red dragon, tyrant magma titan;
21. sleek silver robot fused with dinosaur, omega cyber rex;
22. sleek silver robot fused with demonic jester, black-red execution machine;
23. immortal firebird fused with sleek robot, mechanical raptor phoenix;
24. immortal firebird fused with water spirit, blue heron made of ghost flame.
Row 5:
25. golden sun-mask deity fused with muscular hamster, leonine celestial guardian;
26. golden sun-mask deity fused with carnivorous plant, many-colored ritual mask bloom;
27. silver alien machine fused with radiant fairy, charming psychic extraterrestrial;
28. silver alien machine fused with undead knight, biomechanical armored horse-spirit;
29. water spirit maiden fused with demonic jester, dark siren;
30. water spirit maiden fused with pink rice-cake creature, serene immortal mermaid.
Row 6:
31. pink rice-cake creature fused with undead knight, adorable armored mochi warrior;
32. pink rice-cake creature fused with sleek robot, eight-armed cyber mochi guardian;
33. carnivorous plant fused with radiant fairy, crimson princess flower;
34. carnivorous plant fused with giant moth/worm, pale moth-wing flower monster;
35. red dragon fused with black monolith, rune-covered volcanic dragon fortress;
36. red dragon fused with blue lightning wolf, glacial lightning dragon.

Art direction: mature fantasy, imposing but readable, no gore, no existing franchise character likenesses. Make this a cohesive atlas usable as card illustrations.
```

### `support-card-atlas-v1.jpg`

組み込みImageGenの生成PNGを目視確認後、品質90のJPEGへ変換した。カード順は `growthCards` 5件の後に `breeder-001`〜`breeder-020`。

```text
Create one production-ready square raster sprite atlas for non-monster cards in a dark high-fantasy collectible card battle game.

LAYOUT IS CRITICAL:
- Exactly 5 columns by 5 rows, 25 equal-size square cells in row-major order.
- Each cell is a separate full-bleed vertical-card-compatible illustration (compose the important subject in the center).
- Perfectly aligned cell boundaries; no gutters, no padding, no frames, no separators.
- No text, letters, numbers, symbols, logos, UI, card frames, watermarks, or labels anywhere.
- Consistent premium painterly Japanese dark-fantasy card-art style, cinematic rim lighting, readable silhouettes, rich atmospheric backgrounds.
- These are support/action cards, not monster portraits. Show trainers, hands, equipment, magic, training scenes, or tactical commands.
- Every cell visually distinct. No existing franchise character likenesses.
- Do not make a contact sheet with captions. This must be a seamless crop-ready game asset.

CELL CONTENT IN EXACT ROW-MAJOR ORDER:
Row 1 — growth:
1. life training: determined young monster trainer and creature running uphill at sunrise, vitality aura;
2. attack training: creature smashing a massive stone training pillar with an explosive punch;
3. defense training: creature bracing behind a heavy forged shield under a rain of impacts;
4. attack discipline retreat: mountain dojo, martial master teaching an offensive strike, red-gold ki;
5. defense discipline retreat: waterfall temple, armored master teaching an immovable guard, blue-silver ki.

Row 2 — breeder tactics 1–5:
6. veteran breeder calmly directing several creatures, golden command aura;
7. intimidating tactical commander applying psychological pressure across a battlefield;
8. focused trainer pointing at one ally, concentrated red energy around its next strike;
9. protective trainer raising a luminous barrier around one ally;
10. emergency supply satchel opening with card-like magical provisions flying out, no readable marks.

Row 3 — breeder tactics 6–10:
11. commander ordering a full team charge, all allies glowing with attack energy;
12. trainer granting a tired creature renewed action with a burst of green-gold energy;
13. saboteur issuing interference orders, shadow chains weakening an enemy attack;
14. inorganic specialist reinforcing a stone-and-metal creature’s armor plates;
15. precision engineer revealing weak seams in an enemy shield to ignore defense.

Row 4 — breeder tactics 11–15:
16. magical creator tuning a crystalline artificial creature to spend less energy;
17. arcane controller freezing an enemy in a temporal stun seal;
18. spirit medium granting a spectral creature another immediate action;
19. phantom trainer wrapping an ally in translucent mist so the next attack passes through;
20. demonic war coach empowering one horned ally with crimson attack flames.

Row 5 — breeder tactics 16–20:
21. demon tactician drawing power from several demonic allies into one champion;
22. beast handler converting the pack’s momentum into glowing energy crystals;
23. gentle beast healer restoring a wounded animal companion with warm green light;
24. monster hunter cursing one enemy with black tendrils and a slowing hex;
25. commander uniting multiple giant monsters under a combined red-and-blue power aura.

Art direction: mature fantasy, energetic and dramatic, no gore. Make this a cohesive atlas usable as card illustrations.
```

### `blue-drill-v2.jpg`

ブルードリルだけに確認された重要部位の切れを直すため、組み込みImageGenで専用画像を生成し、JPEGへ変換した。ゲーム内の名称・TP・能力・効果は引き続きHTML描画で、画像には含めていない。

```text
Use case: stylized-concept
Asset type: square monster card illustration for a dark high-fantasy mobile collectible card game
Primary request: create the special fusion monster ブルードリル (Blue Drill), a giant armored burrowing worm fused with a blue lightning wolf
Scene/backdrop: dark subterranean rock cavern with blue electrical arcs and dust
Subject: one powerful blue horned burrowing predator; segmented worm-like armored body, wolf-like ferocity, a prominent drill-shaped horn, icy blue metal scales, electric mane accents
Style/medium: premium painterly Japanese dark-fantasy card art, realistic painted textures, strong silhouette, cinematic rim lighting, saturated cyan-blue accents, high contrast; visually cohesive with a mature fantasy monster atlas
Composition/framing: square portrait; center the entire head, horn, snout, neck and readable upper-body coil; keep every important feature inside a generous 15 percent safe margin on all sides; the horn and snout must be fully visible and must not touch or cross the image edges
Lighting/mood: cold blue lightning against deep charcoal rock, imposing and energetic
Constraints: exactly one monster; full-bleed background; no text, letters, numbers, symbols, logos, UI, card frame, border, caption, watermark; no important anatomy cropped; no existing copyrighted character likeness
Avoid: cut-off head, cut-off horn, cut-off snout, anatomy touching edges, multiple creatures, collage, chibi proportions, photorealistic animal photo
```
