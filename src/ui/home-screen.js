...
export const TUTORIAL_STEPS = Object.freeze([
  ...
  {
    title: '後攻10ターン目で覚醒解禁',
    copy: '後攻が10ターン目に到達すると両者の覚醒が解禁されます。場のモンスター詳細から、別の味方1体を墓地へ送り真の能力を開花します。',
    tokens: [['10', '後攻ターン'], ['醒', '全能力+15'], ['墓', '味方1体を素材'] ],
    tip: '召喚酔い中の味方は素材にできません。覚醒は各プレイヤー1試合に1回で、覚醒したモンスターは合体できません。',
  },
  {
    title: 'ターンとTP',
...
function howToPlayContent(onTutorial) {
  return el('div', { className: 'how-to-play' }, [
    ...
    el('ol', {}, [
      ...
      el('li', { text: 'アリーナは対戦相手選択時に手動操作かオートバトルを選べます。オートでは両方のデッキを同じランクのAIが操作し、結果画面から実際の試合内容をリプレイできます。' }),
...
const HOME_ICON_PATHS = Object.freeze({
  ...
  audio: ['M8 20h8l10-9v26l-10-9H8V20Z', 'M32 18c3 3 3 9 0 12M37 13c6 6 6 16 0 22'],
});
...
function openHomeNotices(champion) {
  openModal({
    ...
    content: el('div', { className: 'home-lobby-modal-copy' }, [
      ...
      el('h3', { text: 'アリーナ・オートバトル追加' }),
      el('p', { text: '格下・同格・格上の候補から、手動操作またはオートバトルを選択できます。結果画面の「試合内容を見る」から実戦リプレイも確認できます。' }),
