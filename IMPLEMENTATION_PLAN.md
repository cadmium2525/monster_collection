...
## Phase 11: サバイバルモード（1.39.0）

- [x] 保存40枚を固定してCPUと連戦するseed駆動ラン
- [x] Silver → Gold → Legend＋無上限の仮想大会内育成による難易度曲線
- [x] Training・修行・習得技と残りLIFEの連戦持越し、5勝ごとのLIFE全回復
- [x] 敗北／任意終了時の冪等なダイヤ・パック券・自己ベスト報酬
- [x] 最高連勝ランキングとFirestore検証ルール／複合index
- [x] トーナメント・アリーナ・サバイバルの独立中断／再開
- [x] ホーム下部サバイバル導線と、王者欄下のトーナメント導線
- [x] 固定seedの対戦結果再現テスト、検証CLI、844×390横画面QA
...
- [x] 特別絵メインの外観を継承する特殊合体特別イラスト48枚と管理者確認UI
- [x] 第2世代6体へ各2種、計12種の特殊合体・特殊特性・通常絵・特別絵を追加
...
- 通常24特性、特殊合体48特性、技効果、ブリーダー52種（既存20＋追加32）
- 40T残LIFE判定
- seeded自動対戦と単体テスト
...
- 保存40枚の種類／TP／名前／登録順による表示ソート（資産ID・保存順は非破壊）
