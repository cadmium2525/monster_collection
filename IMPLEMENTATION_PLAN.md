...
## Phase 10: 無料ブースターとカード資産（1.14.0）

- [x] 奪取限定・ブースター限定・汎用の獲得経路をマスターIDで分離
- [x] 6モン類ブースター、ダイヤ報酬、回数保証、再送冪等性
- [x] 新モンスター6体・54技・特性・修行プール
- [x] 開封前保存、未確認パック再開、Firestore transaction
- [x] 未所属資産、デッキ専用プール、40枚交換UI
- [x] 特別イラスト／Foilの奪取通常絵化
- [x] 通常6枚・特別イラスト24枚のWebP制作と遅延キャッシュ
- [x] 自動テスト、Pagesビルド、文書化
...
- 通常18特性、特殊合体36特性、技効果、ブリーダー40種（既存20＋2026-08-26追加20）
- 40T残LIFE判定
- seeded自動対戦と単体テスト
...
- 自分+CPU15名の16人ブラケット
- 1回戦、2回戦、準決勝、決勝の4試合
- CPU裏試合勝者のTraining/修行をseed付きで生成し、相手ごとの大会内育成として次ラウンドへ持越し
...
- 交換確定時だけ40枚を更新
- 最大5デッキ、自由命名、総TP、変更可能なデッキリーダー、最高到達表示
- 所有経験カードと成功済み特殊合体を永久記録するカード一覧
- 敗退時保存、優勝時だけ次大会資格付与
...
- 実装完了日: 2026-08-24（1.9.1追補: 2026-08-25）
